...
/* Thread-safety (so `--memcheck` works for std.task / std.sync / std.chan
   programs that allocate from worker threads). A statically-initialized lock
   serializes every tracker-table mutation, so concurrent allocs/frees can't
   corrupt the hash table. The frame stack (used only for leak-report
   backtraces) is thread-local instead of locked: each thread keeps its own LS
   call-stack, snapshotted into the entry at alloc time, so backtraces stay
   coherent per thread without serializing enter/leave. The lock uses static
   initializers (SRWLOCK_INIT / PTHREAD_MUTEX_INITIALIZER) so there is no
   init-time race — no lazy lock setup to get wrong. */
#if defined(_WIN32)
#  include <windows.h>
   static SRWLOCK g_mc_lock = SRWLOCK_INIT;
#  define MC_LOCK()        AcquireSRWLockExclusive(&g_mc_lock)
#  define MC_UNLOCK()      ReleaseSRWLockExclusive(&g_mc_lock)
#  define MC_THREAD_LOCAL  __declspec(thread)
#else
#  include <pthread.h>
   static pthread_mutex_t g_mc_lock = PTHREAD_MUTEX_INITIALIZER;
#  define MC_LOCK()        pthread_mutex_lock(&g_mc_lock)
#  define MC_UNLOCK()      pthread_mutex_unlock(&g_mc_lock)
...
/* Thread-local: each worker thread tracks its own LS call-stack. */
static MC_THREAD_LOCAL LsMcFrame  g_frame_stack[LS_MC_FRAME_STACK_MAX];
static MC_THREAD_LOCAL int        g_frame_top = 0;
...
LS_MC_EXPORT void *ls_mc_realloc(void *old_p, size_t new_sz, const LsMcSite *site) {
    ...
    if (!new_p) { MC_UNLOCK(); return NULL; }
...
LS_MC_EXPORT void ls_mc_report(void) {
    MC_LOCK();
    if (g_reported) { MC_UNLOCK(); return; }  /* idempotent: explicit + atexit safe */
    g_reported = 1;
