...
/* Reader-writer lock (RwLock) — a heap SRWLOCK acquired in SHARED (read) or
 * EXCLUSIVE (write) mode: many readers OR one writer. Same heap-handle/move/
 * null-safe-destroy shape as the mutex above (SRWLOCK has no destroy fn). The
 * shared side is what a plain mutex lacks — it lets readers proceed in parallel. */
...
int ls_rwlock_rdlock(void *h) {
    if (h == NULL) return -1;
    AcquireSRWLockShared((SRWLOCK *)h);
    return 0;
}
...
int ls_rwlock_rdunlock(void *h) {
    if (h == NULL) return -1;
    ReleaseSRWLockShared((SRWLOCK *)h);
    return 0;
}
...
/* Condition variable (std.chan blocking Chan). A heap CONDITION_VARIABLE behind
 * an opaque handle (same move-safe shape as the mutex). It pairs with an SRWLOCK
 * mutex handle: ls_cond_wait atomically releases the SRWLOCK (held EXCLUSIVE by
 * ls_mutex_lock) and sleeps until signalled, then re-acquires it before
 * returning. No destroy call needed — just free the block. Lost-wakeup safety is
...
void ls_cond_wait(void *h_cond, void *h_mtx) {
    ...
    /* h_mtx is an SRWLOCK held EXCLUSIVE (ls_mutex_lock uses Exclusive). */
    SleepConditionVariableSRW((CONDITION_VARIABLE *)h_cond,
                              (SRWLOCK *)h_mtx, INFINITE, 0);
}

void ls_cond_signal(void *h) {
    if (h != NULL) WakeConditionVariable((CONDITION_VARIABLE *)h);
}

void ls_cond_broadcast(void *h) {
    if (h != NULL) WakeAllConditionVariable((CONDITION_VARIABLE *)h);
}

void ls_cond_destroy(void *h) {
    if (h != NULL) free(h);   /* CONDITION_VARIABLE has no destroy function */
}
