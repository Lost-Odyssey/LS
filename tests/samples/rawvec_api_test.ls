// rawvec_api_test.ls — comprehensive RawVec(T) API parity with builtin vec.
...
fn main() {
    ...
    v.truncate(2)                        // [10,30]
    check(v.length() == 2 && v.get(1) == 30, "truncate")

    v.resize(5, 0)                       // [10,30,0,0,0]
    check(v.length() == 5 && v.get(4) == 0, "resize grow")
    v.resize(1, 0)                       // [10]
    check(v.length() == 1 && v.get(0) == 10, "resize shrink")

    RawVec(int) c = v.copy()
    c.push(99)
    check(c.length() == 2 && v.length() == 1, "copy is independent")

    v.clear()
    check(v.is_empty(), "clear")
    v.shrink_to_fit()
    check(v.capacity() == 0, "shrink empty -> cap 0")
    ...
    check(s.length() == 3, "str len after pop")

    s.resize(5, f"fill")                 // [NEW,c,b,fill,fill]
    check(s.length() == 5 && s.get(4) == "fill", "str resize grow (clone fill)")
    s.truncate(2)                        // [NEW,c]
    check(s.length() == 2, "str truncate")

    RawVec(string) sc = s.copy()
    check(sc.length() == 2 && sc.get(1) == "c", "str copy")
