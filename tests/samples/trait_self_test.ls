...
interface Addable {
    def add(&self, f64 dx, f64 dy) -> Self
}
...
methods Vec2: Addable {
    def add(&self, f64 dx, f64 dy) -> Vec2 {
...
methods Point {
    def translate(&self, int dx, int dy) -> Self {
        return Point { x: self.x + dx, y: self.y + dy }
    }
}

// Constrained generic using Addable
def shift(T: Addable)(T a, f64 dx, f64 dy) -> T {
    return a.add(dx, dy)
}
