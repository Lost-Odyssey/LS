...
//   - The lifted def (`__closure_N(env, x)`) is callable through the fat ptr
...
def apply1(int x, Adder f) -> int {
    return f(x)
}

def apply2(int x, int y, Pair f) -> int {
    return f(x, y)
}

def pred_check(int x, Predicate f) -> bool {
    return f(x)
}

def produce(Producer f) -> int {
    return f()
}
