// rawvec_functional_p3_test.ls -- RawVec functional parity over Block(T)
...
fn main() {
    ...
    check(nums.find_index(|x| x == 5) == 4, "int find_index")
    match nums.find(|x| x > 4) {
        Some(x) => { check(x == 5, "int find") }
        None => { check(false, "int find none") }
    }
    RawVec(int) big = nums.filter(|x| x > 3)
    check(big.length() == 3 && big[0] == 4 && big[2] == 9, "int filter")
    nums.sort_by(|a, b| a - b)
    check(nums[0] == 1 && nums[1] == 1 && nums[5] == 9, "int sort_by")

    RawVec(string) words = [f"pear", f"kiwi", f"banana", f"plum"]
    ...
    check(words.find_index(|s| s == "banana") == 2, "string find_index")
    match words.find(|s| s.length == 4) {
        Some(w) => { check(w == "pear", "string find") }
        None => { check(false, "string find none") }
    }
    RawVec(string) short = words.filter(|s| s.length == 4)
    check(short.length() == 3 && short[1] == "kiwi", "string filter")
    ...
    RawVec(Person) older = people.filter(|p| p.age >= 30)
    check(older.length() == 2 && older[0].name == "amy", "struct filter")
