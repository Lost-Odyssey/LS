...
def main() {
    ...
    *int p = std.sys.c.malloc(16 * sizeof(int)) as *int
    ...
    std.sys.c.free(p as *u8)

    // ---- struct buffer: padded stride must be correct ----
    *Pt q = std.sys.c.malloc(8 * sizeof(Pt)) as *Pt
    ...
    std.sys.c.free(q as *u8)

    // ---- *u8 byte buffer ----
    *u8 b = std.sys.c.malloc(4) as *u8
    b[0] = 10 as u8
    b[1] = 20 as u8
    b[2] = 30 as u8
    b[3] = 40 as u8
