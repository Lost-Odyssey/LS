// map_test.ls - end-to-end tests for std.core.map Map(K,V)

import std.core.map
import std.core.str

def get_ii(&Map(int, int) m, int key) -> int {
    match m.get(key) {
        Some(v) => { return v }
        None => { return 0 }
    }
...
def get_si(&Map(Str, int) m, Str key) -> int {
    match m.get(key) {
        Some(v) => { return v }
        None => { return 0 }
    }
...
def get_ss(&Map(Str, Str) m, Str key) -> Str {
    match m.get(key) {
        Some(v) => { return v }
        None => { return "" }
    }
