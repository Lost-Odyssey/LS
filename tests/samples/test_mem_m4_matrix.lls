...
def mk_str() -> Str { return "rvalue".upper() }
def mk_item() -> Item { return Item{name: "from-fn".upper(), qty: 7} }
def mk_box() -> Box  { return One("box-rval".upper()) }
def mk_vec() -> Vec(Str) {
    Vec(Str) v = ["a".upper(), "b".upper()]
    return v
}
def mk_map() -> Map(Str, int) {
    Map(Str, int) m = {}
    m.set("k".upper(), 1)
    return m
}
...
def take_str(Str s) -> int {
    @print(s)
    return 1
}

// 接收 struct by-value
def take_item(Item it) -> int {
    @print(it.name)
    return it.qty
}
