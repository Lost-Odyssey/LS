...
methods RawVecS: Clone {
    def clone(&self) -> RawVecS {
        RawVecS out = new_rvs()
...
def new_rvp() -> RawVecP { *Person p = nil; return RawVecP { data: p, len: 0, cap: 0 } }
methods RawVecP {
    def push(&!self, Person x) {
        if self.len >= self.cap {
            int n = 4
            if self.cap > 0 { n = self.cap * 2 }
            self.data = std.sys.c.realloc(self.data as *u8, n * sizeof(Person)) as *Person
            self.cap = n
        }
        self.data[self.len] = x
        self.len = self.len + 1
    }
    def count(&self) -> int { return self.len }
    ...
    def get_full(&self, int i) -> Person { Person pp = self.data[i]; return pp }
    def name_of(&self, int i) -> Str { return self.data[i].name }
    def age_of(&self, int i) -> int { return self.data[i].age }
}
...
def new_rvv() -> RawVecV { *RawVecS p = nil; return RawVecV { data: p, len: 0, cap: 0 } }
methods RawVecV {
    def push(&!self, RawVecS x) {
        if self.len >= self.cap {
            int n = 2
            if self.cap > 0 { n = self.cap * 2 }
            self.data = std.sys.c.realloc(self.data as *u8, n * sizeof(RawVecS)) as *RawVecS
            self.cap = n
        }
        self.data[self.len] = x
        self.len = self.len + 1
    ...
    def row_len(&self, int i) -> int {
        RawVecS inner = self.data[i]     // -> RawVecS.__clone (deep copy)
        return inner.length()
    }
    def row_get(&self, int i, int j) -> Str {
