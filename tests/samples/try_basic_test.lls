...
def double_pos(int x) -> Result(int, Str) {
    int v = try parse_pos(x)
    return Ok(v * 2)
}

def chain(int a, int b, int c) -> Result(int, Str) {
...
def first_even(int x) -> Option(int) {
    match x % 2 {
        0 => Some(x)
        _ => None
    }
...
def double_even(int x) -> Option(int) {
