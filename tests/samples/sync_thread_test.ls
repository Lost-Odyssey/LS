// std.sync — cross-thread MUTUAL EXCLUSION. N workers concurrently push into a
// SHARED global Vec while holding a lock. A Vec push is multi-step (read len,
// store, len++, maybe realloc); without the lock, concurrent pushes race —
// lost elements or a concurrent realloc that corrupts the heap (crash). The
// lock serialises them, so the final length is EXACTLY N*per. Both Mutex and
...
//
// NO --memcheck (tracker not thread-safe — like std.task). Correctness +
