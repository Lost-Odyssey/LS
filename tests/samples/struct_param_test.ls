...
impl Point {
    ...
    static fn create(int x, int y) -> Point {
        Point p
        p.x = x
        p.y = y
        return p
...
impl Resource {
    ...
    static fn create(int val) -> Resource {
        Resource r
        r.id = val
        return r
    }
...
fn take_by_value(Point p) {
    print("  In take_by_value:", p.x, p.y)
}

fn take_resource(Resource r) {
    print("  In take_resource:", r.id)
}

fn identity_point(Point p) -> Point {
    print("  In identity_point:", p.x, p.y)
    return p
}

fn identity_resource(Resource r) -> Resource {
