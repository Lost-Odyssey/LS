...
methods RawVecI {
    def push(&!self, int x) {
        if self.len >= self.cap {
            ...
            self.data = std.sys.c.realloc(self.data as *u8, ncap * sizeof(int)) as *int
            self.cap = ncap
        }
