// rawvec_map_reduce_test.ls -- method-level generic map/reduce on RawVec(T).
...
fn main() {
    ...
    // ── map: int → int ──────────────────────────────────────────────────────
    RawVec(int) nums = [1, 2, 3, 4, 5]
    RawVec(int) doubled = nums.map(int)(|x| x * 2)
    check(doubled.length() == 5, "map int->int len")
    check(doubled[0] == 2 && doubled[2] == 6 && doubled[4] == 10, "map int->int vals")
    check(nums.length() == 5, "map src untouched")

    // ── map: int → string ───────────────────────────────────────────────────
    RawVec(string) strs = nums.map(string)(|x| f"n{x}")
    check(strs.length() == 5, "map int->string len")
    check(strs[0] == "n1" && strs[2] == "n3" && strs[4] == "n5", "map int->string vals")

    // ── map: string → string ────────────────────────────────────────────────
    RawVec(string) words = [f"apple", f"banana", f"cherry"]
    RawVec(string) upper_words = words.map(string)(|s| s.upper())
    check(upper_words.length() == 3, "map str->str len")
    ...
    RawVec(int) lens = words.map(int)(|s| s.length)
    check(lens.length() == 3, "map str->int len")
    check(lens[0] == 5 && lens[1] == 6 && lens[2] == 6, "map str->int vals")

    // ── map: int → struct ───────────────────────────────────────────────────
    RawVec(int) ages = [20, 30, 40]
    RawVec(Person) people = ages.map(Person)(|a| Person { name: f"p{a}", age: a })
    check(people.length() == 3, "map int->struct len")
    check(people[0].name == "p20" && people[0].age == 20, "map int->struct first")
    check(people[2].name == "p40" && people[2].age == 40, "map int->struct last")

    // ── map: struct → string ────────────────────────────────────────────────
    RawVec(string) names = people.map(string)(|p| p.name)
    check(names.length() == 3, "map struct->string len")
    check(names[1] == "p30", "map struct->string val")

    // ── map on empty vec ────────────────────────────────────────────────────
    RawVec(int) empty = {}
    RawVec(string) empty_mapped = empty.map(string)(|x| f"x{x}")
    check(empty_mapped.length() == 0, "map empty")

    // ── reduce: int sum ─────────────────────────────────────────────────────
    RawVec(int) r_nums = [1, 2, 3, 4, 5]
    ...
    // ── reduce: int max ─────────────────────────────────────────────────────
    RawVec(int) unordered = [3, 1, 4, 1, 5, 9, 2, 6]
    ...
    RawVec(string) parts = [f"hello", f"world", f"ls"]
