// fft_real_dct_smoke.ls — std.sci.fft Phase 4: rfft/irfft + DCT-II/III.
...
def cplxify(Vec(f64) r) -> Vec(Complex(f64)) {
    Vec(Complex(f64)) v = []
    int i = 0
...
def dct_naive(Vec(f64) x) -> Vec(f64) {
    int bigN = x.len()
