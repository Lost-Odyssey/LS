// Phase E.1: closure body captured Vec passed to def — clone fallback.
...
def vec_len(Vec(int) v) -> int {
    return v.len()
}

def count_keys(&Map(Str, int) m) -> int {
    return m.len()
}
