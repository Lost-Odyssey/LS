...
def check_int(int got, int want, Str name) {
    if got == want { return }
...
def mk_owned_str(Str tail) -> Option(Str) {
    Str base = "owned-"
    return Some(base + tail)
}

def mk_owned_vec(int n) -> Option(Vec(int)) {
    Vec(int) v = []
    v.push(n)
    v.push(n + 1)
    return Some(v)
...
def get_some(int x) -> Option(int) {
    if x > 0 { return Some(x) }
    return None
}

def get_ok(int x) -> Result(int, Str) {
    if x > 0 { return Ok(x) }
    return Err("bad")
}
...
methods Box {
    def maybe(&self) -> Option(int) {
        if self.n > 0 { return Some(self.n) }
        return None
    }

    def result(&self) -> Result(int, Str) {
