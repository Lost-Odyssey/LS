...
// the static Complex(f64).zero() inside tensor's generic methods (zeros free def +
...
def near(f64 a, f64 b) -> bool { f64 d = a - b; if d < 0.0 { d = 0.0 - d } return d < 0.0000001 }

def main() {
    Vec(int) sh = [2]

    // zeros(Complex(f64)) — T.zero() dispatch inside a generic FREE def (exercises
