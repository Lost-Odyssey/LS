// tensor_phase0_test.ls — std.tensor 堆 Tensor 阶段 0 地基探针（plan_ndarray_stdlib.md §-1）
//
...
fn imat_make(int rows, int cols) -> IMat {
    int n = rows * cols
...
impl IMat {
    fn rows(&self) -> int { return self.rows }
    fn cols(&self) -> int { return self.cols }
    fn size(&self) -> int { return self.size }

    // checked 读（三层安全模型的 checked 层；at!/set! unchecked 留阶段 1）
    fn at(&self, int i, int j) -> int {
    ...
    fn __clone(&self) -> IMat { return self.copy() }

    // 析构：释放自有 buffer。
    fn __drop() { c.free(self.data as *u8) }
}

fn check(bool ok, Str l) {
    if ok { print(f"ok {l}") } else { print(f"FAIL {l}") }
}

// 纯 LS 函数收 *int + n：模拟 C 端按 int* 接收基址求和（同 ABI）。
fn psum(*int p, int n) -> int {
    int s = 0
