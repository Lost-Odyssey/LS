# test_struct_readthrough.cmake — transient has_drop struct field read-through driver.
#
# Regression for the leak that occurred when reading THROUGH a has_drop struct
# field (e.g. `o.inner.items.length`): the intermediate field object `o.inner`
# was deep-cloned as an unregistered owned temporary, leaking its vec/map/string
# heap. Fix borrows `&o.inner` via GEP instead of cloning for read-through.
#
# Runs the sample three ways and asserts correctness ("READTHROUGH PASS", never
# "FAIL") plus memory safety (0 leak / 0 double-free / 0 invalid free under JIT
# memcheck):
#   1. JIT run            — value correctness (incl. consume-independence + mutate-through)
#   2. AOT compile + run  — value correctness on the native path
#   3. JIT --memcheck     — no transient-clone leak
#
...
if(NOT LS_EXE OR NOT SAMPLE)
    message(FATAL_ERROR "test_struct_readthrough.cmake requires LS_EXE and SAMPLE")
endif()
if(NOT TEST_NAME)
    set(TEST_NAME "struct_readthrough")
endif()
...
if(NOT _jit_out MATCHES "READTHROUGH PASS")
    message(FATAL_ERROR "${TEST_NAME} JIT missing 'READTHROUGH PASS':\n${_jit_out}")
endif()
...
if(NOT _aot_out MATCHES "READTHROUGH PASS")
    message(FATAL_ERROR "${TEST_NAME} AOT missing 'READTHROUGH PASS':\n${_aot_out}")
endif()
