...
set(LS      "${LS_EXE}")
set(SRC     "${CMAKE_CURRENT_LIST_DIR}/samples/vec_functional_v1_test.lls")
set(OUT_EXE "${CMAKE_BINARY_DIR}/vec_v1_test.exe")
set(MC_EXE  "${CMAKE_BINARY_DIR}/vec_v1_mc_test.exe")
...
if(NOT jit_rc EQUAL 0)
    message(FATAL_ERROR "V.1 JIT run failed (rc=${jit_rc}):\n${jit_err}")
endif()
if(jit_out MATCHES "FAIL")
    message(FATAL_ERROR "reported a FAIL:\n${jit_out}")
endif()
if(NOT jit_out MATCHES "ALL PASS")
    message(FATAL_ERROR "V.1 JIT: expected 'ALL PASS', got:\n${jit_out}")
endif()
message(STATUS "V.1 JIT: OK")
...
if(NOT mc_rc EQUAL 0)
    message(FATAL_ERROR "V.1 JIT memcheck failed (rc=${mc_rc}):\n${mc_err}")
endif()
if(NOT mc_err MATCHES "0 leak")
    message(FATAL_ERROR "V.1 JIT memcheck: expected 0 leaks:\n${mc_err}")
endif()
message(STATUS "V.1 JIT memcheck: OK (0 leaks)")
...
if(NOT aot_crc EQUAL 0)
    message(FATAL_ERROR "V.1 AOT compile failed (rc=${aot_crc}):\n${aot_cerr}")
endif()
execute_process(
    COMMAND "${OUT_EXE}"
...
if(NOT aot_out MATCHES "ALL PASS")
    message(FATAL_ERROR "V.1 AOT: expected 'ALL PASS', got (rc=${aot_rc}):\n${aot_out}")
endif()
message(STATUS "V.1 AOT: OK")
...
if(NOT mc_crc EQUAL 0)
    message(FATAL_ERROR "V.1 AOT memcheck compile failed (rc=${mc_crc}):\n${mc_cerr}")
endif()
execute_process(
    COMMAND "${MC_EXE}"
...
if(NOT mc_aot_out MATCHES "ALL PASS")
    message(FATAL_ERROR "V.1 AOT memcheck: expected 'ALL PASS', got:\n${mc_aot_out}")
endif()
if(NOT mc_aot_err MATCHES "0 leak")
    message(FATAL_ERROR "V.1 AOT memcheck: expected 0 leaks:\n${mc_aot_err}\n${mc_aot_out}")
endif()
message(STATUS "V.1 AOT memcheck: OK (0 leaks)")

message(STATUS "V.1 all passed")
