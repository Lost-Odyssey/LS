# test_struct_byval.cmake — by-value has_drop struct argument ownership driver.
#
# Regression for the double-free that occurred when a struct with a has_drop
# field (vec/map/string/nested struct) was passed BY VALUE: the call-site clone
# (emit_struct_clone_val) used to leave vec/map fields as shallow copies sharing
# the caller's buffer, so callee scope_drop + caller scope_drop double-freed it.
#
# Runs the sample three ways and asserts correctness ("BYVAL PASS", never "FAIL")
# plus memory safety (0 leak / 0 double-free / 0 invalid free under JIT memcheck):
#   1. JIT run            — value correctness
#   2. AOT compile + run  — value correctness on the native path
#   3. JIT --memcheck     — no double-free of the shared field heap
#
...
if(NOT LS_EXE OR NOT SAMPLE)
    message(FATAL_ERROR "test_struct_byval.cmake requires LS_EXE and SAMPLE")
endif()
if(NOT TEST_NAME)
    set(TEST_NAME "struct_byval")
endif()
...
if(NOT _jit_out MATCHES "BYVAL PASS")
    message(FATAL_ERROR "${TEST_NAME} JIT missing 'BYVAL PASS':\n${_jit_out}")
endif()
...
if(NOT _aot_out MATCHES "BYVAL PASS")
    message(FATAL_ERROR "${TEST_NAME} AOT missing 'BYVAL PASS':\n${_aot_out}")
endif()
