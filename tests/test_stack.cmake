# test_stack.cmake — std.stack (generic container) correctness + memcheck driver.
#
# Exercises the FIRST cross-module generic container end to end: a sample that
# `import std.stack` and instantiates Stack(int) / Stack(string) at its own call
# sites (the Step 0 cross-module generic path). Asserts:
#   1. JIT run            — value correctness ("STACK PASS", no "FAIL")
#   2. AOT compile + run  — value correctness on the native path
#   3. JIT --memcheck     — 0 leak / 0 double-free / 0 invalid free
#      (the probe for generic struct + vec(T) field auto-drop/clone)
#
...
#   STDLIB    — repo root, set as LS_HOME so `import std.stack` resolves
cmake_minimum_required(VERSION 3.20)

if(NOT LS_EXE OR NOT SAMPLE)
    message(FATAL_ERROR "test_stack.cmake requires LS_EXE and SAMPLE")
endif()
if(STDLIB)
...
if(NOT _jit_out MATCHES "STACK PASS")
    message(FATAL_ERROR "${TEST_NAME} JIT missing 'STACK PASS':\n${_jit_out}")
endif()
...
if(NOT _aot_out MATCHES "STACK PASS")
    message(FATAL_ERROR "${TEST_NAME} AOT missing 'STACK PASS':\n${_aot_out}")
endif()
