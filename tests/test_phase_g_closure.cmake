# test_phase_g_closure.cmake — Phase G (Block env deep-clone, L-007) driver.
#
# Copying a Block out of a vec element / struct field / map value deep-clones the
# closure env (cg_emit_block_env_clone + per-closure __env_clone_N), so the
# destination owns an independent env. Runs the self-verifying sample three ways:
#   1. JIT run            — value correctness ("G PASS", never "G FAIL")
...
if(NOT LS_EXE OR NOT SAMPLE)
    message(FATAL_ERROR "test_phase_g_closure.cmake requires LS_EXE and SAMPLE")
endif()
if(NOT TEST_NAME)
    set(TEST_NAME "phase_g_closure")
endif()
...
if(NOT _jit_out MATCHES "G PASS")
    message(FATAL_ERROR "${TEST_NAME} JIT missing 'G PASS':\n${_jit_out}")
endif()
...
if(NOT _aot_out MATCHES "G PASS")
    message(FATAL_ERROR "${TEST_NAME} AOT missing 'G PASS':\n${_aot_out}")
endif()
