...
set(LS "${LS_EXE}")
set(SRC_OK "${CMAKE_CURRENT_LIST_DIR}/samples/rawvec_kid_lazy_test.ls")
set(SRC_BAD "${CMAKE_CURRENT_LIST_DIR}/samples/rawvec_kid_missing_eq_fail.ls")
set(OUT_EXE "${CMAKE_BINARY_DIR}/rawvec_kid.exe")
if(STDLIB)
...
if(NOT ar EQUAL 0 OR NOT ao MATCHES "KID LAZY PASS" OR ao MATCHES "FAIL ")
    message(FATAL_ERROR "rawvec_kid AOT: bad output (rc=${ar}):\n${ae}\n${ao}")
endif()

execute_process(COMMAND "${LS}" compile "${SRC_BAD}" -o "${CMAKE_BINARY_DIR}/rawvec_kid_bad.exe"
    RESULT_VARIABLE br ERROR_VARIABLE be OUTPUT_VARIABLE bo TIMEOUT 30)
if(br EQUAL 0)
    message(FATAL_ERROR "rawvec_kid negative unexpectedly compiled:\n${bo}")
endif()
set(bad_out "${be}\n${bo}")
if(NOT bad_out MATCHES "requires T: Eq" OR NOT bad_out MATCHES "does not implement Eq")
    message(FATAL_ERROR "rawvec_kid negative: expected missing Eq where-bound error, got:\n${bad_out}")
endif()

message(STATUS "rawvec_kid all passed")
