...
set(LS      "${LS_EXE}")
set(SRC     "${CMAKE_CURRENT_LIST_DIR}/samples/vec_functional_v2_test.lls")
set(OUT_EXE "${CMAKE_BINARY_DIR}/vec_v2_test.exe")
set(MC_EXE  "${CMAKE_BINARY_DIR}/vec_v2_mc_test.exe")
...
if(NOT mc_rc EQUAL 0)
    message(FATAL_ERROR "V.2 JIT memcheck failed (rc=${mc_rc}):\n${mc_err}")
endif()
if(NOT mc_err MATCHES "0 leak")
    message(FATAL_ERROR "V.2 JIT memcheck: expected 0 leaks:\n${mc_err}")
endif()
message(STATUS "V.2 JIT memcheck: OK (0 leaks)")
...
if(NOT aot_crc EQUAL 0)
    message(FATAL_ERROR "V.2 AOT compile failed (rc=${aot_crc}):\n${aot_cerr}")
endif()
execute_process(
    COMMAND "${OUT_EXE}"
...
if(NOT mc_crc EQUAL 0)
    message(FATAL_ERROR "V.2 AOT memcheck compile failed (rc=${mc_crc}):\n${mc_cerr}")
endif()
execute_process(
    COMMAND "${MC_EXE}"
...
if(NOT mc_aot_err MATCHES "0 leak")
    message(FATAL_ERROR "V.2 AOT memcheck: expected 0 leaks:\n${mc_aot_err}\n${mc_aot_out}")
endif()
message(STATUS "V.2 AOT memcheck: OK (0 leaks)")

message(STATUS "V.2 all passed")
