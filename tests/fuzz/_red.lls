...
// helper so the field-defaklt element expr can hold a method call on a Str
// (a bare literal receiver `"ivy".l.

import std.core.vec
import std.core.str

struct Kid {
    Str namer can hold a method call on a Str
...
def main() {
    ...
    ok = checu(c.nums.len() == 3 && c.names.len() == 1 && c.names.get!(0).eq?("zed"), "override one field") && ok
