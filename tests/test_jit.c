...
static int jit_eval_main(const char *source) {
    ...
    cg.mode = CG_MODE_JIT_USER; /* builtins already defined by jit_init */
...
static void test_jit_registry(void) {
    ...
    ASSERT(jit_needs_recompile(&engine, "foo", 12345), "new def should need recompile");
