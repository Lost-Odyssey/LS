...
| VR-LIM-018 | **跨模块** match binder 上 `Vec(T)` generic 方法不可解析 | 2026-06-08 迁移 `json_e2e_test.ls` 时发现。**根因已定位（2026-06-08）**：① **不是** key 不匹配——两侧 `impl_key_of_type` 都是 `'Vec(int)'`；② 真因是消费方模块的 `impl_registry["Vec(int)"]` **只注册了「定义方模块实际调用过」的方法**（如 `push`/`reserve`/`copy`/`__from_list`，由构造触发），而定义方从未调用的 `len()`/`get`/`__index` **不在注册表**；③ checker 方法调用路径 `method_is_static(ds_key,name)` 返回 -1 时**直接报错，不尝试从 `impl(T) Vec(T)` 模板按需实例化补注册**。**同模块永远工作**（本地实例化经 `checker_instantiate_struct`→`instantiate_impl_method_types` 一次性注册全部方法）；**仅跨模块**（binder 类型来自导入 enum 的 payload，实例化发生在别处）触发。属 F6b（跨模块泛型 Vec）同一族根因。 | 绕行：`items.len`（字段，无括号）+ 外部索引（`entries.keys()`）。**修复方向**：method-call 路径 `si==-1` 且 base 模板存在时，从模板实例化缺失方法补注册后重试（需 base+type_args，TYPE_STRUCT 当前无此元数据 → 需侧表记录实例化 args，且导入路径也要填充）。建议并入 F6 一并修。 |
