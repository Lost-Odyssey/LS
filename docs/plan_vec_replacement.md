...
| ~~VR-LIM-013~~ | ~~`Vec(Option(T))` 这类嵌套泛型实参仍有替换边界~~ | ✅ **已修复（F4，2026-06-08）**。根因：type-alias 不是按栈解析。嵌套泛型复用同一参数名（`struct Slots(T)`/`Ring(T)` 的字段 `Vec(Option(T))`：`Slots` 先 push `T→int`，实例化 Vec 的方法体时再 push `Vec`/`impl(T)` 的 `T→Option(int)`），但 `find_type_alias` 从前往后取**第一个**匹配 → 命中外层 `T→int` → Vec 方法体里元素类型错成 `int`。修法：`find_type_alias` 改为**从后往前**遍历，内层 `T` 正确遮蔽外层。 | 验证：`std.ring` 已迁移 `Vec(Option(T))` backing buffer，`test_ring`（含 Ring(string) has_drop）JIT+AOT+memcheck 0/0/0；ctest 168/168。 |
...
| VR-LIM-015 | ~~`Vec(T)` generic 方法的 by-value 参数不标记 named var 为 moved~~ → **非缺陷，是一致行为** | 2026-06-08 迁移 `test_mem_m5_neg_push` 时发现：`Vec(string).push(s)` 后 `s` 仍 live。**实证复核（mv1~mv6）**：所有用户函数/方法（具体 struct、string、泛型 struct `Vec` 整体、泛型方法 `T`）by-value 参数**一律 CLONE，源保持 live**；只有「变量绑定 `b=a`」与「内建 `vec.push`」才 MOVE。故 `Vec` by-value=clone **已与泛型 struct 完全一致**，内建 `vec.push` 的 move 才是不一致的特例。 | **不要**给 `Vec.push` 加 move 标记（会破坏泛型 struct 一致性）。负向 move 测试改用变量绑定 move 载体（见 [plan_phase3_remove_builtin_vec.md](plan_phase3_remove_builtin_vec.md) P3-0b）。若想让 by-value move-type 参数真 move = 语言级另案，影响全部 struct。 |
...
| ~~VR-LIM-017~~ | ~~`Vec(Block(...))` 不兼容~~ | ✅ **已修复（F5，2026-06-08）**。完整 Block 生命周期穿过纯 LS 容器需四处协同：① **checker**：泛型 `T x` 参数单态化到 Block 时不标 `is_borrow`（显式 `Block(..) f` 参数才是 shared-env 借用），使 `push` 体 `self.data[i]=x` 合法；② **codegen** `cg_block_source_is_aliased`：用户 `Vec(Block)` 的 `[i]`/`get`/`first`/`last` copy-out 在 **bind 点** deep-clone env（丢弃的 rvalue 如 `v[i](arg)` 借用不克隆 → 不泄漏）；③ **codegen** move-into-container ABI：闭包字面量传给 `push`/`set`/`insert`/`__index_set`/`__from_list`/`extend` 时消费 temp env（容器接管，调用方不再 free，否则悬垂 → AOT UAF 崩溃）；④ **codegen** `emit_drop_value` 加 `TYPE_BLOCK` 分支，使 `__drop_at(self.data[i])` 释放元素 env。 | 验证：`closure_g.ls` 迁移到 `Vec(Block)`，`test_phase_g_closure` JIT+AOT+memcheck 0/0/0；另测 push/get/index/for-in/discard 全 0/0/0；ctest 168/168。 |
...
| ~~VR-LIM-018~~ | ~~**跨模块** match binder 上 `Vec(T)` generic 方法不可解析~~ | ✅ **已修复（F6，2026-06-08，分支 `feat/xmodule-generic-instantiation`）**。根因：导入模块用**独立 Checker 实例**，消费方 checker 从没本地实例化过 `Vec(int)`（binder 类型是导入 enum payload 的现成 Type\*），其 `impl_registry` 为空。**三处协同修复**：① **codegen**：导入模块函数内局部 `Vec(T)=[lit]` 在 G1.5 前发射，`__from_list` 未声明 → 静默跳过 → 零值 Vec（`len()=0`/段错误）→ 改 `cg_declare_pending_generic_method` 前向声明（F1 兄弟 bug，真正的崩溃源）。② **checker**：`TYPE_STRUCT` 加 generic 元数据（base/args/impl_node/tp_names）+ `ensure_generic_struct_impls_local`——方法调用 `si==-1` 时从模板本地补注册签名。③ **checker**：`find_struct_template_idx_pull`——消费方未直接 `import std.vec` 时，从已检查的已加载模块按需 transitive 拉取 `Vec`/`VecIter` 模板；仅本地 miss 触发，不破坏同名跨模块歧义检测。 | 验证：`json_e2e_test` 还原真方法调用（去 `.len` 字段绕行）；新增 `test_xmod_generic`（消费方不 import std.vec，binder 调 `len()`/`get()`/`[i]`/for-in）JIT+AOT+memcheck 0/0/0；ctest 168/168。 |
