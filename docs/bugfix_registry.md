...
| BF-042 | 2026-05-30 | WRONG | 全局 vec 字面量初始化 `vec(int) g = [1,2,3]` 读回为空（`sum=0`）。`emit_global_var_init` 只特判 `TYPE_ARRAY`+`AST_ARRAY_LIT`，`TYPE_VECTOR` 落到泛型 else（store `codegen_expr(array_lit)`——数组聚合而非堆 vec），全局 vec 结构 {data,len,cap} 留零。预存缺陷（根全局亦坏，与模块无关），借 codegraph `context` 一次调用定位。修复：emit_global_var_init 新增 POD-元素 vec 分支，在 global 指针上 push 建 vec（镜像局部 vec 字面量路径）；`EMIT_GLOBAL_CLEANUP` 加 vec 分支 cap>0 free(data)。限 POD 元素（int/f*/bool/char/ptr，无需逐元素 drop）；非 POD 元素见 BF-043 | codegen (emit_global_var_init / 全局 cleanup) | 本文 BF-042 |
| BF-043 | 2026-05-30 | WRONG/LEAK | BF-042 续：owned-元素全局 vec（`vec(string)` / `vec(has_drop struct/enum)`）。init 同样读空；且即便修了 init，退出清理只 free(data) 会泄漏每个元素的堆资源。修复：emit_global_var_init vec 分支去掉 POD 限制 + 加 string 元素所有权处理（temp move / IDENT clone，镜像局部路径）；新增 `emit_global_vec_cleanup` 助手——退出时逐元素 drop（复用 `emit_vec_elem_drop_at`）后 free(data)，`EMIT_GLOBAL_CLEANUP` vec 分支改为调它。全局 has_drop 元素的 `__drop` 副作用退出时正常执行，memcheck 0 leak/0 dfree。顺修 `emit_global_vec_cleanup` 里 `LLVMGetBasicBlockParent` 返回值误声明为 `LLVMBasicBlockRef`（应 `LLVMValueRef`，C4133） | codegen (emit_global_var_init / emit_global_vec_cleanup) | 本文 BF-043 |
| BF-044 | 2026-05-30 | CRASH | has_drop struct 的 `vec[i].field` 出现在短路 `&&`/`\|\|` 条件里 → `Instruction does not dominate all uses`。根因：`codegen_short_circuit` 在条件块 `sc.rhs` 里求值 RHS，`vec[i].field` 的 M-4.5 spill clone alloca + temp_drop 建在此块，但 flush 留到外层语句边界（merge 后，不被 sc.rhs 支配）→ drop 的 alloca 不支配。修复：RHS 求值后、跳 merge 前，在 sc.rhs 块内 `cg_flush_temps` 就地 flush RHS temps（RHS 结果是 bool，不持有资源，安全；LHS temps 在 entry 块支配一切、外层 flush 即可）。借 codegraph `context` 定位 flush/short-circuit 机制。`&&`/`\|\|`/三重链/局部+全局 vec/memcheck 全过 | codegen (codegen_short_circuit) | 本文 BF-044 |
| BF-045 | 2026-05-30 | WRONG | **AOT** 下：owned string 作函数/方法参数 `s` → 存入返回的 struct 字段（`fn of(string s)->S{return S{a:s}}`）或直接 `return s` → 调用方读得垃圾（JIT 侥幸 UAF 给对值）。根因：string 参数 prologue 标 cap=-2（borrow，别名调用方缓冲），但 codegen 符号**未标 `is_borrowed`**（不像 vec/map/Block），导致 `cg_store_owned` 走 move 分支原样存入字段（而非 clone 分支 `emit_string_clone_val`，后者会克隆 cap=-2）；字段逃逸后调用方释放临时 → 悬垂。修复：①string 参数标 `is_borrowed=true`（fn_decl param 注册处）→ 存入容器/字段走克隆；②`AST_RETURN` 返回借用 string 参数时 clone（与 BF-041 全局返回同一克隆点）。最小复现转为 `test_bf045_string_param`（JIT+AOT+memcheck）。注：纯根 struct 即复现，与模块无关 | codegen (fn_decl string param is_borrowed / AST_RETURN) | 本文 BF-045 |
| BF-046 | 2026-05-30 | LEAK | `m.set(k, N{...})` 传**临时 has_drop struct/enum 右值**时泄漏（16B/元素）。根因：map.set 是 **deep-copy 语义**（clone 进 node），但 `codegen_map_method` 的 set 路径对值实参**既不 move 也不 drop**——原临时 struct 仍持有 owned 字段（string），克隆后无人释放（对比 vec.push 是 move、map 字面量路径有 flush）。命名值 `N v; m.set(k,v)` 干净（v 由 scope drop）；POD 值干净。修复：set 调用后，若值实参是临时 has_drop struct/enum 右值（非 IDENT），spill 到 entry alloca + `cg_push_temp_drop` 注册语句末 drop（map 持独立 clone，不 double-free）。顺修 `emit_auto_drop_fn` 早返（函数已存在时）未给该 Type 对象设 `drop_fn` → 第二个 Type 实例 drop_fn 留 NULL 的潜在 skip。预存（根 struct 即复现），B-6 压测发现。`test_bf046_map_struct_val` | codegen (codegen_map_method set / emit_auto_drop_fn) | 本文 BF-046 |
