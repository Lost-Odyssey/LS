...
| BF-044 | 2026-05-30 | CRASH | **待修**：has_drop struct 的 `vec[i].field` 出现在短路 `&&`/`\|\|` 条件里 → `Instruction does not dominate all uses`（M-4.5 spill clone 的 temp_drop alloca 建在 && 右侧条件块，语句末 flush 处不被支配）。局部与全局 vec 均复现，与全局 vec 无关。规避：先 `string t = vec[i].field` 绑定到局部再比较。根因在 M-4.5 temp_drop 与短路分支的交互（BF-040 邻域） | codegen (field access on AST_INDEX + 短路布尔) | 待修 |
