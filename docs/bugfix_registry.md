...
| BF-045 | 2026-05-30 | **待修** WRONG | **AOT** 下：owned string 作**函数/方法参数** `s` → 存入返回的 struct 字段（`fn of(string s) -> S { return S { a: s, ... } }`）→ 调用方读 `.a` 得垃圾（如 `r=m`，1 字符）。JIT 正确（HELLO）。传**静态** string 则 AOT 也正确 → 触发条件是 **owned string 参数 + 构造进返回 struct 的字段**。预存（纯根 struct 即复现，与模块无关），B-6 压测发现。疑为 string-参数 ABI × struct-by-value 返回（sret）的 codegen 交互。最小复现：`struct S{string a int n} fn of(string s)->S{return S{a:s,n:1}} main: S x=of("h".upper()); print(x.a)` | codegen (string param ABI / struct return) | 待修 |
