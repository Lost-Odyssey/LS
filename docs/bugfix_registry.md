...
| BF-046 | 2026-05-30 | **待修** LEAK | `map(K, has_drop_struct)`：map 清理时**不 drop 结构体值的 owned 字段**（如 string）→ 泄漏（16 字节/元素）。`map(string,string)` 干净（string 值正确 drop），但值为含 string 的 struct 时漏；只 `set` 不读也漏。预存（根 struct 即复现，与模块无关），B-6 压测发现。根因疑为 map 的值-drop 路径对 TYPE_STRUCT 值未调用 struct drop。最小复现：`struct N{string s int n} map(string,N) m; m.set("k".upper(), N{s:"v".upper(),n:1})` → 1 leak | codegen (map value drop) | 待修 |
