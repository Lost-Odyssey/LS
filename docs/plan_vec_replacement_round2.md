...
### F4 — `Vec(Option(T))` 嵌套泛型实参替换（VR-LIM-013，ring 前置）✅ 已修复（2026-06-08）
- **现象**：`Vec(Option(T)).get/set` 单态化后元素类型错推成内层 `T`，报
  `cannot initialize 'tmp'(int) with Option(int)` / `cannot assign *string to *Option(string)`。
- **根因（实测，与原猜测不同）**：不是「实参替换不递归」——`Vec(Option(int))` 直接用完全正常。
  真因是 **type-alias 解析不是栈式**。嵌套泛型复用同名参数 `T`：`struct Slots(T)`/`Ring(T)`
  的字段 `Vec(Option(T))`，实例化 `Slots(int)` 时 push 别名 `T→int`，在其方法体里又触发
  `Vec(Option(int))` 方法实例化、push `Vec`/`impl(T)` 的 `T→Option(int)`；但 `find_type_alias`
  从前往后取**第一个**匹配 → 命中外层 `T→int` → Vec 方法体（`T tmp = self.data[i]`）里
  `T` 错成 `int`，与 `self.data[i]`（`*T`→`Option(int)`）冲突。
- **修法**（单点 1 行）：`find_type_alias` 改为**从后往前**遍历，内层（最后注册）`T` 遮蔽外层。
...
- **F6a ✅ 已修复（路 B，2026-06-08）**：`map(K,V).keys()`/`values()` 改为返回 `Vec(K)`/`Vec(V)`。
  关键洞察——builtin vec `LsVec{ptr,i32,i32}` 与 `Vec(T){*T,int,int}` **布局完全相同**，且 LS
  `malloc`/`realloc`/`free` 都转发到同一 `ls_mc_*` 堆。所以 codegen 把现成的桶遍历结果（ls_mc
  分配的 `{data,len,cap}`）直接 reinterpret 成 `Vec(T)` 聚合返回，调用方 `Vec(T).__drop` 释放，
  堆/元素兼容。checker 返回类型 `vec(K)→Vec(K)`（靠 F6 transitive 模板拉取解析，调用方即使没
  直接 import std.vec 也行）。**顺带修了一个 F6/VR-LIM-018 潜伏 bug**：跨模块 Vec(K) binder 方法
  调用经 `ensure_*` 复用方法注册时，幂等守卫早返回会让**另一个** Vec(K) 实例漏设 `has_drop` →
  其值静默不析构（泄漏）。修法：守卫早返前从 impl 模板把 `has_drop`/`has_user_drop` 传播到本
  struct_type 实例。回归测试扩到 `test_xmod_generic`（Vec(string) has_drop + 跨模块 binder 方法）。
  验证：`test_map_keys` 迁移到 Vec、`json_e2e` 真方法调用、`test_modtype_memcheck` 三绿，ctest 168/168。
  - ~~（原路线）~~两条路线择一——
