...
### F2 — `v.pop()` 丢弃返回值时 rvalue `Option(T)` temp 泄漏（VR-LIM-014）
- **现象**：`v.pop()` 作语句（非赋值右值）时，`Option(string)` rvalue 内 `Some` 的
  string buffer 不 `__drop`，泄漏。`Option(T) _ = v.pop()` 则 0/0/0。
- **方向**：`AST_EXPR_STMT` / 语句位置的 rvalue 求值，对 has_drop enum 临时未注册
  `cg_push_temp_drop`。复用 L-012 已成熟的 owned rvalue enum temp 析构路径，扩到
  「方法调用返回的 has_drop enum 被丢弃」场景。
- **验收**：`vec_string_test.ls` 的 `v.pop()` 还原为裸语句，memcheck 0/0/0。
- **附带收益**：stack.ls 的 `self.data.pop()`（stack.ls:41 丢弃）直接受益。

### F3 — 整 has_drop struct rvalue 喂 print/builtin 泄漏（VR-LIM-008 残留）
- **现象**：`print(vp[0])`（整个 `Person` rvalue 直接做 builtin 实参）泄漏
  `Vec(Person).get` 内的 string clone；而 `v[i].field` / `f(v[i])` / `Person p=v[i]`
  已 0/0/0（test_vec_owndrop）。
- **方向**：builtin/print 实参位置的 has_drop struct rvalue（来自 `Vec.get` 返回）未注册
  temp drop。在 print/builtin 实参求值处，对 owned struct rvalue 注册 `cg_push_temp_drop`
  （区别于命名变量经借址不进此路径）。
- **验收**：`vec_struct_clone_test.ls` 的 `print(vp[0])` 路径还原，memcheck 0/0/0。
...
- **F6b 跨模块同名 Vec 单态化冲突**（modtype_memcheck）：两个模块各自 `Vec(int)` 单态化
