...
| 2026-05-24 | — | **fix: Bug #9-11 — vec.first/last/get 缺少 has-drop enum clone 分支**。三处"ok_bb: deep-clone"逻辑均只处理了 string 和 struct，漏掉 enum → 浅拷贝 → double-free。各加 `emit_enum_clone_val` 分支；`enum_has_drop_vec_test.ls` 补 J/K/L 三节覆盖，memcheck 0/0/0，ctest 55/55 通过。 |
| 2026-05-24 | — | **fix: Phase E-3 — has-drop enum var_decl clone + 零初始化**。① `AST_VAR_DECL` 对 has-drop enum 的 IDENT 初始化（`Value b = a`）补加 `emit_enum_clone_val`（与 struct 处理对称）；② 无 init 的 has-drop enum 变量补零初始化，防止 scope cleanup 访问垃圾 payload。新增测试 `enum_var_decl_test.ls`（7 场景）+ `enum_nested_vec_test.ls`（6 场景），memcheck 全部 0/0/0，ctest 55/55 通过。 |
