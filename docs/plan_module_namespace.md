...
> 状态：Part 1 ✅ + B-1 ✅ + B-2 ✅ + B-3 ✅ + B-4 核心 ✅（限定类型/歧义）｜ B-4.1（同名带方法）+ B-5（enum 变体）+ B-6 待实现 ｜ 关联：L-009 / L-009.1（`docs/plan_l009_mangling.md`）
...
> **遗留 B-4.1（同名 struct/enum **带方法** 跨模块）**：当前两模块各 `impl Widget { fn val }`
> 同时导入 → checker 报 `conflicting method 'val'`（**非崩溃**）。根因：`impl_registry` 与
> ~10 个方法解析点（`find_method`/`method_is_static`/调用分派，`checker.c:533/621/632/650` +
> `4703~4842`）按**裸 struct 名**索引，importer 注册导入 impl（`checker.c:8050`）亦按裸名。
> 修法：全部改用类型唯一名（`strukt/enom.llvm_name ?: name`）做 impl 注册/查找 key；静态限定调用
> `A.Widget.make()` 需 parser/checker 支持三段限定。宽改动、风险中，单列 B-4.1。
