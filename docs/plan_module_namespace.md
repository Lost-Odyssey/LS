...
> 状态：**模块命名空间全部完成** ✅ — Part 1 + B-1~B-6 全 ✅（B-5 经类型上下文、B-6 综合 memcheck）。
> B-6 压测另发现两个与模块无关的预存 bug：BF-045（AOT string-参数→struct字段）/ BF-046（map struct-值泄漏），见 `bugfix_registry.md`。
> 关联：L-009 / L-009.1（`docs/plan_l009_mangling.md`）
...
#### Step B-2：struct/enum Type 名烘焙模块前缀 ✅ 已完成（2026-05-30）
- **实现方式**：在 `types.h` 的 `strukt`/`enom` 加 `llvm_name` 字段。Checker 端
  `check_struct_decl`/`check_enum_decl` 中，若 `c->module_name != NULL`，调用新 helper
  `checker_module_type_llvmname` 计算前缀名并赋给 `llvm_name`（bare name 仍存 `name`，
  checker 注册表/impl_registry/type_name 显示均用裸名，零破坏）。Codegen 端新增
  `struct_llvm_name`/`enum_llvm_name_of` helpers，替换 `type_to_llvm`、`codegen_struct_decl`、
  `codegen_enum_decl`、`emit_auto_drop_fn`（struct+enum）、`emit_auto_enum_clone_fn`、
  成员 drop 查找、方法调用分派共 12 处使用裸名的 LLVM 命名点。
- importer 注册（`checker.c:7902/7913`）：**仍按裸名注册 + 保留 B-1 冲突检测**（单模块无变化）。
- `type_name()` 返回裸 `strukt.name` → 错误信息依然显示用户友好的裸名，无需额外处理。
- **验证**：ctest 79/79 全绿；JIT+AOT+memcheck 三重通过。
- **新测**：`test_modtype_ns` B-2 组——单模块 struct/enum 行为不变（JIT+AOT+memcheck）。

#### Step B-3：codegen 方法 / drop / clone 名跟随前缀 ✅ 已完成（2026-05-30）
- `codegen_impl_decl` 和 `codegen_impl_trait_decl`：`impl_decl.name` 在 `current_emit_module
  != NULL` 时用 `cg_module_fn_symbol` 前缀化，使方法 qualified name 变 `mod_a__Config.method`。
- 静态方法调用 `Widget.make()` 时 `obj_type==NULL`，新增按裸名搜索 `ctx->struct_types` 找
  `llvm_name` 的逻辑，保证 `Widget.make` 正确解析为 `mod_a__Widget.make`。
- drop/clone 合成函数名通过 `struct_llvm_name`/`enum_llvm_name_of` 已随 B-2 前缀化。
- **验证**：单模块 struct + 方法 + 自定义 `__drop`（计数器断言），JIT+AOT+memcheck clean。
- **新测**：`test_modtype_ns` B-3 组——方法调用 + has_drop struct + user __drop 计数器。
- 注：B-2+B-3 完成后，**同名 struct 在各自模块内部已是不同 LLVM 类型**，只是 importer 仍按 B-1 拦同时导入。

#### Step B-4：类型位置限定语法 `mod.Type` / `alias.Type` ✅ 核心完成（2026-05-30）｜ 方法情形见 B-4.1
本步是放开同名共存的关键，也是唯一有 parser 歧义的一步。
**已实现并三重验证**（`test_modtype_qualified`：两模块同名不同布局 `Config` 用 `A.Config`/`B.Config`
共存 → `a=1 b=60`，JIT+AOT+memcheck clean；裸名歧义 → 清晰报错；单模块裸名零回归）：
1~5 全部落地（AST `named.module` + parser parse_type 吃 `.IDENT` 链 + starts_var_decl
`IDENT(.IDENT)*IDENT(同行)` 前瞻 + resolve_type_node 走模块导出表 + importer 歧义改为标记）。
`test_modtype_conflict`（原 B-1）已重定向为 B-4 语义（导入不再报错，裸名使用才报，错误仍含
"multiple imported modules"）。

> **B-4.1（同名 struct/enum **带方法** 跨模块）✅ 已完成（2026-05-30）**：新增 `impl_key_of_type`
> （`strukt/enom.llvm_name ?: name`），impl_registry 的注册（本地 impl/__drop/impl_trait/导入 impl）
> 与查找（实例/静态/enum 方法分派 + find_method + 字段方法）全部改用类型唯一名做 key。导入 impl 用
> `type_module_find_export` 取本模块的类型（避免歧义裸名）。根/builtin 类型 llvm_name==NULL → key
> 仍为裸名，零回归。验证：两模块同名 Widget/Kind 带实例+静态+enum 方法共存 → `va=30 vb=105 ra=2
> rb=40`，JIT+AOT+memcheck clean（`test_modtype_methods`）。3 段静态调用 `A.Widget.make()` 经
> 模块成员解析亦正常。ctest 81/81。
...
#### Step B-5：enum 跨模块 + 变体 ✅ 实质完成（2026-05-30，实测驱动）
**实测结论**：B-2~B-4.1 落地后，跨模块同名 enum 的**变体已经由类型上下文正确解析**，无需额外
代码改动。验证（含**同名 enum + 同名变体**，两模块各 `enum Res { Ok, Err }`）：
- 声明上下文 `A.Res x = Ok` / `B.Res y = Ok` → 各自正确（不是取第一个）。
- 实参上下文 `A.code(Ok)` / `B.code(Ok)` → 各自正确。
- match：对 `B.Res` 值用裸 `Ok`/`Err` pattern → 按 scrutinee 类型正确解析。
- 类型安全：`A.Res r = Done`（Done∉A.Res）→ 报错。
- 不同名变体跨模块各归各。
全部 JIT+AOT+memcheck clean（`test_modtype_enum_variants`）。

> **未做（低价值，可选）**：显式限定变体语法 `A.Res.Ok`（当前报 "has no method 'Ok'"）。
> 因 LS **强制显式类型**，裸变体在声明/实参/返回/match 处**处处有类型上下文**驱动解析，
> 真正"无上下文且歧义"的场景几乎不存在；且当前是清晰报错而非静默错。故不实现，留作
> B-5.1 备选（需 parser 在表达式位置解析 `mod.Enum.Variant` + checker 变体限定解析）。

#### Step B-6：has_drop 同名 struct/enum 跨模块综合 memcheck ✅ 已完成（2026-05-30）
- 综合压测（`test_modtype_memcheck`）：两模块同名 has_drop struct（owned string 字段，不同布局）+ 方法
  + vec 容器（含 Phase H 深拷贝取出）+ enum payload + 跨模块返回，限定类型消歧。JIT+AOT+memcheck **0 leak/0 dfree**。
- **模块命名空间功能本身全部正确**。压测期间**额外发现两个与模块无关的预存 bug**（均在根 struct 上复现）：
  - **BF-045（待修）**：owned string 参数 → 构造进返回 struct 字段 → AOT 字段损坏（JIT 正确）。
  - **BF-046（待修）**：`map(K, has_drop struct)` 值的 owned 字段在 map 清理时泄漏。
  B-6 测试已避开这两个触发点（结构体内联构造、不用 struct-值 map），保持干净；两 bug 见 `bugfix_registry.md`。
