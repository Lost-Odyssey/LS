...
static LLVMBasicBlockRef emit_string_free_inline(CodegenContext *ctx, LLVMValueRef str_alloca);
...
static int ls_str_alloc_cap(int slen) {
    ...
    if (need <= LS_MIN_STR_CAP) return LS_MIN_STR_CAP;
...
/* Emit string cleanup inline: loads cap, creates free_bb if needed.
   After this call, builder is at free_bb (or a new cont_bb if no free needed).
   WARNING: Only call ONCE at the end of a block, or ensure subsequent code
   branches to the returned block. For multiple cleanups, use emit_cleanup_to instead. */
static LLVMBasicBlockRef emit_string_free_inline(CodegenContext *ctx, LLVMValueRef str_alloca) {
    LLVMTypeRef str_type = ls_string_type(ctx);
    LLVMTypeRef i32_type = LLVMInt32TypeInContext(ctx->context);
    ...
    /* Conditional branch: if owned, go to free_bb, else go to skip_bb */
    LLVMBuildCondBr(ctx->builder, is_owned, free_bb, skip_bb);

    /* Free path: call free(data), then branch to cont */
    ...
    /* Skip path: branch to cont (no free needed) */
    LLVMPositionBuilderAtEnd(ctx->builder, skip_bb);
    LLVMBuildBr(ctx->builder, cont_bb);

    /* Return cont_bb - caller should NOT add instructions here */
    return cont_bb;
}

/* Simple wrapper - positions at a new block after cleanup.
   WARNING: For multiple cleanups in same scope, use emit_cleanup_to instead. */
static void emit_string_free(CodegenContext *ctx, LLVMValueRef str_alloca) {
    LLVMBasicBlockRef cont = emit_string_free_inline(ctx, str_alloca);
...
static LLVMValueRef cg_push_temp_string(CodegenContext *ctx, LLVMValueRef str_val) {
    if (ctx->current_fn == NULL) return str_val;  /* global context: no alloca */
    LLVMTypeRef str_type = ls_string_type(ctx);
...
static CgSymbol *get_string_var_symbol(AstNode *expr, CodegenContext *ctx) {
    ...
    if (expr->kind != AST_IDENT) return NULL;
    Type *t = expr->resolved_type;
    if (t == NULL || t->kind != TYPE_STRING) return NULL;
            ...
            if (sym->type->kind == TYPE_STRING) {
                ...
                if (elem_type->kind == TYPE_STRING) {
                    ...
                    for (int ei = 0; ei < arr_size; ei++) {
                        ...
                        LLVMBasicBlockRef cont = emit_string_free_inline(ctx, elem_ptr);
                        LLVMPositionBuilderAtEnd(ctx->builder, cont);
                        idx++;
                    }
...
static void emit_struct_drop_cond(CodegenContext *ctx, LLVMValueRef drop_ptr,
    ...
    if (!struct_type->as.strukt.has_drop) return;
...
static LLVMBasicBlockRef emit_struct_drop_separate(CodegenContext *ctx, LLVMValueRef drop_ptr,
                                                   Type *struct_type, LLVMValueRef moved_flag) {
    if (struct_type == NULL || struct_type->kind != TYPE_STRUCT) return NULL;
    if (!struct_type->as.strukt.has_drop) return NULL;

    LLVMValueRef drop_fn = (LLVMValueRef)struct_type->as.strukt.drop_fn;
    if (drop_fn == NULL) return NULL;
...
static void emit_auto_drop_fn(CodegenContext *ctx, Type *struct_type) {
    if (struct_type == NULL || struct_type->kind != TYPE_STRUCT) return;
    if (struct_type->as.strukt.drop_fn != NULL) return;  /* already has user drop */
    if (!struct_type->as.strukt.has_drop) return;       /* nothing to drop */
    if (struct_type->as.strukt.field_count == 0) return;
    ...
    for (int i = struct_type->as.strukt.field_count - 1; i >= 0; i--) {
        ...
        /* Skip pointer types */
        if (field_type->kind == TYPE_POINTER) continue;
...
static LLVMTypeRef type_to_llvm(CodegenContext *ctx, Type *t) {
    if (t == NULL) return LLVMVoidTypeInContext(ctx->context);
    ...
    case TYPE_NIL:    return LLVMPointerTypeInContext(ctx->context, 0);
    case TYPE_STRING: return ls_string_type(ctx);
    case TYPE_OBJECT: return LLVMPointerTypeInContext(ctx->context, 0);
    case TYPE_POINTER: return LLVMPointerTypeInContext(ctx->context, 0);
    case TYPE_LIB:    return LLVMPointerTypeInContext(ctx->context, 0);
    case TYPE_ARRAY:
...
static LLVMValueRef codegen_lvalue_ptr(CodegenContext *ctx, AstNode *node) {
    ...
    if (node->kind == AST_FIELD) {
        ...
        if (stype == NULL || stype->kind != TYPE_STRUCT) return NULL;
        ...
        if (is_ptr) {
            ...
            if (ptr_alloca == NULL) return NULL;
        ...
        if (struct_llvm == NULL) struct_llvm = type_to_llvm(ctx, stype);
    ...
    if (node->kind == AST_INDEX) {
        ...
        if (obj_type == NULL || obj_type->kind != TYPE_ARRAY) return NULL;
...
static const char *printf_fmt_for_type(Type *t) {
    ...
    switch (t->kind) {
    case TYPE_INT:
    case TYPE_I32:    return "%d";
    case TYPE_I8:     return "%d";
    case TYPE_I16:    return "%d";
    case TYPE_I64:    return "%lld";
    case TYPE_U8:     return "%u";
    case TYPE_U16:    return "%u";
    case TYPE_U32:    return "%u";
    case TYPE_U64:    return "%llu";
    case TYPE_F32:    return "%f";
    case TYPE_F64:    return "%f";
    case TYPE_BOOL:   return "%s";
    case TYPE_STRING: return "%s";
    case TYPE_POINTER:return "%p";
    case TYPE_OBJECT: return "%p";
...
static void codegen_print_array(CodegenContext *ctx, AstNode *arg) {
    ...
    if (arr_type == NULL || arr_type->kind != TYPE_ARRAY) return;
...
static LLVMValueRef codegen_print_call(CodegenContext *ctx, AstNode *node) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (arg->kind == AST_FORMAT_STRING) {
            ...
            for (int j = 0; j < arg->as.format_string.expr_count; j++) {
                ...
                if (val == NULL) { free(printf_args); return NULL; }
        ...
        if (i > 0) {
            ...
            if (fmt_len < 1020) fmt_buf[fmt_len++] = ' ';
        }
    ...
    /* Append newline */
    if (fmt_len < 1022) fmt_buf[fmt_len++] = '\n';
    fmt_buf[fmt_len] = '\0';
...
static LLVMValueRef codegen_format_string(CodegenContext *ctx, AstNode *node) {
    ...
    for (int i = 0; i < expr_count; i++) {
        ...
        LLVMValueRef val = codegen_expr(ctx, expr);
        if (val == NULL) { free(vals); return NULL; }
    ...
    sp_args[0] = buf;
    sp_args[1] = fmt_str;
    for (int i = 0; i < val_count; i++) sp_args[2 + i] = vals[i];
...
static LLVMBasicBlockRef emit_vec_elem_drop_at(CodegenContext *ctx,
                                                 ...
                                                 Type *elem_type,
                                                 int idx_suffix) {
    if (elem_type == NULL) return LLVMGetInsertBlock(ctx->builder);
    (void)idx_suffix;

    if (elem_type->kind == TYPE_STRING) {
        /* elem_ptr is a *LsString.
           emit_string_free_inline returns cont_bb but leaves builder at skip_bb.
           Use emit_string_free which also positions builder at cont_bb. */
...
static LLVMValueRef codegen_addr_of(CodegenContext *ctx, AstNode *node) {
    ...
    if (node->kind == AST_FIELD) {
        ...
        if (!struct_llvm) struct_llvm = type_to_llvm(ctx, struct_type);
...
static LLVMValueRef codegen_expr(CodegenContext *ctx, AstNode *node) {
    ...
    case AST_UNARY: {
        ...
        case TOKEN_AMP: {
            ...
            if (node->as.unary.operand->kind == AST_IDENT) {
                ...
                if (sym) return sym->value; /* alloca IS the address */
            }
    ...
    case AST_BINARY: {
        ...
        case TOKEN_PLUS:
            ...
            if (is_fp) return LLVMBuildFAdd(ctx->builder, left, right, "fadd");
            return LLVMBuildAdd(ctx->builder, left, right, "add");
        case TOKEN_MINUS:
            if (is_fp) return LLVMBuildFSub(ctx->builder, left, right, "fsub");
            return LLVMBuildSub(ctx->builder, left, right, "sub");
        case TOKEN_STAR:
            if (is_fp) return LLVMBuildFMul(ctx->builder, left, right, "fmul");
            return LLVMBuildMul(ctx->builder, left, right, "mul");
        case TOKEN_SLASH:
            if (is_fp) return LLVMBuildFDiv(ctx->builder, left, right, "fdiv");
            if (is_signed_int) return LLVMBuildSDiv(ctx->builder, left, right, "sdiv");
            return LLVMBuildUDiv(ctx->builder, left, right, "udiv");
        case TOKEN_PERCENT:
            if (is_signed_int) return LLVMBuildSRem(ctx->builder, left, right, "srem");
            return LLVMBuildURem(ctx->builder, left, right, "urem");

        /* Bitwise */
        case TOKEN_AMP:   return LLVMBuildAnd(ctx->builder, left, right, "and");
        case TOKEN_PIPE:   return LLVMBuildOr(ctx->builder, left, right, "or");
        case TOKEN_CARET:  return LLVMBuildXor(ctx->builder, left, right, "xor");
        case TOKEN_LSHIFT: return LLVMBuildShl(ctx->builder, left, right, "shl");
        case TOKEN_RSHIFT:
            if (is_signed_int) return LLVMBuildAShr(ctx->builder, left, right, "ashr");
        ...
        case TOKEN_EQ:
            ...
            if (is_fp) return LLVMBuildFCmp(ctx->builder, LLVMRealOEQ, left, right, "feq");
        ...
        case TOKEN_NEQ:
            ...
            if (is_fp) return LLVMBuildFCmp(ctx->builder, LLVMRealONE, left, right, "fne");
            return LLVMBuildICmp(ctx->builder, LLVMIntNE, left, right, "ne");
        case TOKEN_LT:
            if (is_fp) return LLVMBuildFCmp(ctx->builder, LLVMRealOLT, left, right, "flt");
            if (is_signed_int) return LLVMBuildICmp(ctx->builder, LLVMIntSLT, left, right, "slt");
            return LLVMBuildICmp(ctx->builder, LLVMIntULT, left, right, "ult");
        case TOKEN_GT:
            if (is_fp) return LLVMBuildFCmp(ctx->builder, LLVMRealOGT, left, right, "fgt");
            if (is_signed_int) return LLVMBuildICmp(ctx->builder, LLVMIntSGT, left, right, "sgt");
            return LLVMBuildICmp(ctx->builder, LLVMIntUGT, left, right, "ugt");
        case TOKEN_LEQ:
            if (is_fp) return LLVMBuildFCmp(ctx->builder, LLVMRealOLE, left, right, "fle");
            if (is_signed_int) return LLVMBuildICmp(ctx->builder, LLVMIntSLE, left, right, "sle");
            return LLVMBuildICmp(ctx->builder, LLVMIntULE, left, right, "ule");
        case TOKEN_GEQ:
            if (is_fp) return LLVMBuildFCmp(ctx->builder, LLVMRealOGE, left, right, "fge");
            if (is_signed_int) return LLVMBuildICmp(ctx->builder, LLVMIntSGE, left, right, "sge");
    ...
    case AST_CALL: {
        ...
        if (total_argc > 0) {
            ...
            for (int i = 0; i < user_argc; i++) {
                ...
                if (args[i + arg_offset] == NULL) { free(args); return NULL; }
            }
    ...
    case AST_FIELD: {
        ...
        if (obj_type && obj_type->kind == TYPE_VECTOR) {
            ...
            if (strcmp(field, "length") == 0 || strcmp(field, "capacity") == 0) {
                ...
                if (vec_val == NULL) vec_val = codegen_expr(ctx, obj_node);
                if (vec_val == NULL) return NULL;
    ...
    case AST_INDEX: {
        ...
        /* vec(T)[index] — load data ptr from LsVec, GEP into it, load element */
        if (obj_type && obj_type->kind == TYPE_VECTOR) {
            LLVMValueRef vec_alloca = NULL;
            ...
            if (index == NULL) return NULL;
            if (LLVMTypeOf(index) != i64_type)
                index = LLVMBuildSExtOrBitCast(ctx->builder, index, i64_type, "vi.idx");
            LLVMValueRef gep = LLVMBuildGEP2(ctx->builder, elem_llvm, data_ptr,
                                              &index, 1, "vi.ep");
            return LLVMBuildLoad2(ctx->builder, elem_llvm, gep, "vi.elem");
        }
    ...
    case AST_ARRAY_LIT: {
        ...
        if (arr_type == NULL || arr_type->kind != TYPE_ARRAY) return NULL;
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (elems[i] == NULL) { free(elems); return NULL; }
            if (!LLVMIsConstant(elems[i])) all_const = false;
        }
...
static void codegen_stmt(CodegenContext *ctx, AstNode *node) {
    ...
    case AST_VAR_DECL: {
        ...
        if (node->as.var_decl.init) {
            ...
            if (var_type->kind == TYPE_ARRAY &&
                ...
                if (init) LLVMBuildStore(ctx->builder, init, alloca);
    ...
    case AST_ASSIGN: {
        ...
        if (node->as.assign.target->kind == AST_IDENT) {
            ...
            if (node->as.assign.op == TOKEN_ASSIGN) {
                ...
                if (result) LLVMBuildStore(ctx->builder, result, sym->value);
                ctx->temp_string_count = temp_mark;
            }
            ...
            if (obj_type && obj_type->kind == TYPE_VECTOR) {
                ...
                /* fixed array[i] = val */
                if (obj_type == NULL || obj_type->kind != TYPE_ARRAY) return;
            ...
            if (ptr) LLVMBuildStore(ctx->builder, val, ptr);
            ctx->temp_string_count = temp_mark;
        }
        break;
    }
    ...
    case AST_RETURN: {
        ...
        if (node->as.return_stmt.value &&
            ...
            if (ret_type->kind == TYPE_STRING || ret_type->kind == TYPE_STRUCT) {
                ...
                if (sym) return_alloca = sym->value;
            }
        }
    ...
    case AST_WHILE: {
        ...
        if (cond) LLVMBuildCondBr(ctx->builder, cond, body_bb, end_bb);
    ...
    case AST_FOR: {
        ...
        if (is_array_iter) {
            ...
            if (arr_ptr == NULL) { pop_scope(ctx); break; }
        } else if (is_vec_iter) {
            ...
            if (vec_alloca == NULL) { pop_scope(ctx); break; }
        ...
        if (start_val == NULL || end_val == NULL) { pop_scope(ctx); break; }
        ...
        if (is_array_iter) {
            LLVMBuildStore(ctx->builder, start_val, idx_var);
            {
                ...
                if (lvsym) lvsym->is_borrowed = true;
            }
        } else if (is_vec_iter) {
            LLVMBuildStore(ctx->builder, start_val, idx_var);
            {
...
static void emit_drop_field_cleanup(CodegenContext *ctx) {
    ...
    if (fn_len < 7 || strcmp(fn_name + fn_len - 7, ".__drop") != 0) return;
    ...
    if (len >= sizeof(struct_name)) len = sizeof(struct_name) - 1;
    strncpy(struct_name, fn_name, len);
    struct_name[len] = '\0';

    Type *st = find_struct_ls_type(ctx, struct_name);
    if (st == NULL || st->kind != TYPE_STRUCT) return;
...
static void codegen_fn_decl(CodegenContext *ctx, AstNode *node) {
    ...
    bool is_instance_method = (node->as.fn_decl.impl_struct_name != NULL
                               && !node->as.fn_decl.is_static);

    Type *fn_type_ml = node->resolved_type;
    if (fn_type_ml == NULL || fn_type_ml->kind != TYPE_FUNCTION) return;
    ...
    if (node->as.fn_decl.body == NULL) return;
            ...
            if (strcmp(orig_name, "__drop") == 0) {
                ...
                if (st != NULL && user_fn != NULL) {
                    ...
                    for (int fi = st->as.strukt.field_count - 1; fi >= 0; fi--) {
                        ...
                        if (ft == NULL) continue;
                        if (ft->kind == TYPE_POINTER) continue;
...
static void codegen_ffi_init(CodegenContext *ctx, AstNode *ast) {
    ...
    for (int i = 0; i < ast->as.program.decl_count; i++) {
        if (ast->as.program.decls[i]->kind == AST_LOAD_LIB) lib_count++;
    }
    ...
    for (int i = 0; i < ast->as.program.decl_count; i++) {
        ...
        if (decl->kind != AST_LOAD_LIB) continue;
...
static void codegen_decl(CodegenContext *ctx, AstNode *node) {
    if (node == NULL) return;
    switch (node->kind) {
    case AST_FN_DECL:       codegen_fn_decl(ctx, node); break;
    case AST_STRUCT_DECL:   codegen_struct_decl(ctx, node); break;
    case AST_IMPL_DECL:     codegen_impl_decl(ctx, node); break;
    case AST_EXTERN_FN:     codegen_extern_fn(ctx, node); break;
    case AST_MODULE_DECL:
    case AST_IMPORT_DECL:   break; /* no codegen needed */
    case AST_LOAD_LIB:      codegen_load_lib(ctx, node); break;
    default:
...
int codegen_compile(CodegenContext *ctx, AstNode *ast,
    ...
    if (ast == NULL || ast->kind != AST_PROGRAM) return -1;
    ...
    if (registry) {
        for (int m = 0; m < registry->count; m++) {
            ...
            if (mod_ast == NULL || mod_ast->kind != AST_PROGRAM) continue;
    ...
    /* Generate __ls_global_init for global variables with initializers
       (includes both main module and imported module globals) */
    {
        bool has_global_init = false;

        /* Check main module globals */
        for (int i = 0; i < ast->as.program.decl_count; i++) {
            AstNode *decl = ast->as.program.decls[i];
            if (decl->kind == AST_VAR_DECL && decl->as.var_decl.init) {
                has_global_init = true;
                break;
            }
        }

        /* Check imported module globals */
        if (!has_global_init && registry) {
            for (int m = 0; m < registry->count && !has_global_init; m++) {
                AstNode *mod_ast = registry->modules[m].ast;
                if (mod_ast == NULL || mod_ast->kind != AST_PROGRAM) continue;
                for (int i = 0; i < mod_ast->as.program.decl_count; i++) {
                    AstNode *decl = mod_ast->as.program.decls[i];
        ...
        if (has_global_init) {
            LLVMTypeRef void_type = LLVMVoidTypeInContext(ctx->context);
            LLVMTypeRef init_fn_type = LLVMFunctionType(void_type, NULL, 0, 0);
            LLVMValueRef init_fn = LLVMAddFunction(ctx->module, "__ls_global_init",
                                                     init_fn_type);
            ...
            LLVMValueRef saved_fn = ctx->current_fn;
            ctx->current_fn = init_fn;
            /* Isolate temp slots: global_init runs in its own function context */
            int saved_temp_count = ctx->temp_string_count;
            ctx->temp_string_count = 0;

            /* Helper macro: emit init code for a single VAR_DECL with initializer */
            #define EMIT_GLOBAL_VAR_INIT(decl) do { \
                Type *var_type = (decl)->resolved_type; \
                if (var_type == NULL) break; \
                LLVMValueRef global = LLVMGetNamedGlobal(ctx->module, \
                                                          (decl)->as.var_decl.name); \
                ...
                if (var_type->kind == TYPE_ARRAY && \
                    ...
                    if (const_arr) { \
                        ...
                        for (int j = 0; j < count; j++) { \
                            LLVMValueRef elem_val = codegen_expr(ctx, \
                                lit->as.array_lit.elements[j]); \
                            if (elem_val == NULL) continue; \
            ...
            /* Initialize imported module globals first */
            if (registry) {
                for (int m = 0; m < registry->count; m++) {
                    AstNode *mod_ast = registry->modules[m].ast;
                    if (mod_ast == NULL || mod_ast->kind != AST_PROGRAM) continue;
                    for (int i = 0; i < mod_ast->as.program.decl_count; i++) {
                        AstNode *decl = mod_ast->as.program.decls[i];
                        if (decl->kind != AST_VAR_DECL || !decl->as.var_decl.init) continue;
                        EMIT_GLOBAL_VAR_INIT(decl);
                    }
                }
            }

            /* Initialize main module globals */
            for (int i = 0; i < ast->as.program.decl_count; i++) {
                AstNode *decl = ast->as.program.decls[i];
                if (decl->kind != AST_VAR_DECL || !decl->as.var_decl.init) continue;
                EMIT_GLOBAL_VAR_INIT(decl);
       ...
       Called just before main() returns so global strings don't leak at program exit. */
    {
        ...
        if (has_global_cleanup) {
            ...
            #define EMIT_GLOBAL_CLEANUP(decl) do { \
                if ((decl)->kind != AST_VAR_DECL || !(decl)->resolved_type) break; \
                LLVMValueRef gv = LLVMGetNamedGlobal(ctx->module, (decl)->as.var_decl.name); \
                if (!gv) break; \
                if ((decl)->resolved_type->kind == TYPE_STRING) { \
                    emit_string_free(ctx, gv); \
                } else if ((decl)->resolved_type->kind == TYPE_VECTOR) { \
                    /* vec global cleanup: load cap, if > 0 drop elements + free data */ \
                    Type *vec_type_gc = (decl)->resolved_type; \
                    Type *elem_gc     = vec_type_gc->as.vec.elem; \
                    LLVMTypeRef vt_gc = ls_vec_type(ctx); \
                    LLVMTypeRef i32_gc = LLVMInt32TypeInContext(ctx->context); \
                    LLVMTypeRef i64_gc = LLVMInt64TypeInContext(ctx->context); \
                    LLVMValueRef vval  = LLVMBuildLoad2(ctx->builder, vt_gc, gv, "gcv.v"); \
                    LLVMValueRef cap_gc = LLVMBuildExtractValue(ctx->builder, vval, 2, "gcv.cap"); \
                    LLVMValueRef len_gc = LLVMBuildExtractValue(ctx->builder, vval, 1, "gcv.len"); \
                    LLVMValueRef dat_gc = LLVMBuildExtractValue(ctx->builder, vval, 0, "gcv.dat"); \
                    LLVMValueRef z32    = LLVMConstInt(i32_gc, 0, 0); \
                    LLVMValueRef hasbuf = LLVMBuildICmp(ctx->builder, LLVMIntSGT, cap_gc, z32, "gcv.hb"); \
                    LLVMValueRef gv_cfn  = LLVMGetBasicBlockParent(LLVMGetInsertBlock(ctx->builder)); \
                    LLVMBasicBlockRef gvf_bb = LLVMAppendBasicBlockInContext(ctx->context, gv_cfn, "gcv.free"); \
                    LLVMBasicBlockRef gvd_bb = LLVMAppendBasicBlockInContext(ctx->context, gv_cfn, "gcv.done"); \
                    LLVMBuildCondBr(ctx->builder, hasbuf, gvf_bb, gvd_bb); \
                    LLVMPositionBuilderAtEnd(ctx->builder, gvf_bb); \
                    bool eneed = elem_gc && (elem_gc->kind == TYPE_STRING || \
                        (elem_gc->kind == TYPE_STRUCT && elem_gc->as.strukt.has_drop)); \
                    if (eneed) { \
                        LLVMTypeRef el_gc = type_to_llvm(ctx, elem_gc); \
                        LLVMBuilderRef tb3 = LLVMCreateBuilderInContext(ctx->context); \
                        LLVMBasicBlockRef fn_e = LLVMGetEntryBasicBlock(gv_cfn); \
                        LLVMValueRef fi3 = LLVMGetFirstInstruction(fn_e); \
                        if (fi3) LLVMPositionBuilderBefore(tb3, fi3); \
                        else     LLVMPositionBuilderAtEnd(tb3, fn_e); \
                        LLVMValueRef ei_gc = LLVMBuildAlloca(tb3, i32_gc, "gcv.ei"); \
                        LLVMDisposeBuilder(tb3); \
                        LLVMBuildStore(ctx->builder, z32, ei_gc); \
                        LLVMBasicBlockRef glc = LLVMAppendBasicBlockInContext(ctx->context, gv_cfn, "gcv.lc"); \
                        LLVMBasicBlockRef glb = LLVMAppendBasicBlockInContext(ctx->context, gv_cfn, "gcv.lb"); \
                        LLVMBasicBlockRef gle = LLVMAppendBasicBlockInContext(ctx->context, gv_cfn, "gcv.le"); \
                        LLVMBuildBr(ctx->builder, glc); \
                        LLVMPositionBuilderAtEnd(ctx->builder, glc); \
                        LLVMValueRef ci = LLVMBuildLoad2(ctx->builder, i32_gc, ei_gc, "gcv.ci"); \
                        LLVMValueRef cmp2 = LLVMBuildICmp(ctx->builder, LLVMIntSLT, ci, len_gc, "gcv.lt"); \
                        LLVMBuildCondBr(ctx->builder, cmp2, glb, gle); \
                        LLVMPositionBuilderAtEnd(ctx->builder, glb); \
                        LLVMValueRef ci64 = LLVMBuildSExt(ctx->builder, ci, i64_gc, "gcv.ci64"); \
                        LLVMValueRef ep2  = LLVMBuildGEP2(ctx->builder, el_gc, dat_gc, &ci64, 1, "gcv.ep"); \
                        emit_vec_elem_drop_at(ctx, ep2, elem_gc, 0); \
                        LLVMBasicBlockRef aft = LLVMGetInsertBlock(ctx->builder); \
                        if (!LLVMGetBasicBlockTerminator(aft)) { \
                            LLVMValueRef one32 = LLVMConstInt(i32_gc, 1, 0); \
                            LLVMValueRef ni2 = LLVMBuildAdd(ctx->builder, ci, one32, "gcv.ni"); \
                            LLVMBuildStore(ctx->builder, ni2, ei_gc); \
                            LLVMBuildBr(ctx->builder, glc); \
                        } \
                        LLVMPositionBuilderAtEnd(ctx->builder, gle); \
                        LLVMBuildBr(ctx->builder, gvd_bb); \
                    } else { \
                        /* No element cleanup needed - skip element cleanup loop and go directly to free */ \
                        LLVMPositionBuilderAtEnd(ctx->builder, gvf_bb); \
                    } \
                    /* Free data in gvf_bb (after element cleanup or directly if no cleanup needed) */ \
                    LLVMPositionBuilderAtEnd(ctx->builder, gvf_bb); \
                    { \
                        LLVMValueRef ffn = LLVMGetNamedFunction(ctx->module, "free"); \
                        LLVMTypeRef fft  = LLVMGlobalGetValueType(ffn); \
                        LLVMBuildCall2(ctx->builder, fft, ffn, &dat_gc, 1, ""); \
                        LLVMBuildBr(ctx->builder, gvd_bb); \
                    } \
                    LLVMPositionBuilderAtEnd(ctx->builder, gvd_bb); \
                } \
            } while(0)
       ...
       drop functions generated before container structs that depend on them. */
    {
        ...
        while (progress) {
            ...
            for (int i = 0; i < ctx->struct_type_count; i++) {
                ...
                if (st == NULL || st->kind != TYPE_STRUCT) continue;
                if (!st->as.strukt.has_drop) continue;
                if (st->as.strukt.drop_fn != NULL) continue; /* user-defined or already generated */
    ...
    for (int i = 0; i < ast->as.program.decl_count; i++) {
        ...
        case AST_LOAD_LIB:
        case AST_VAR_DECL:
            /* Already handled in Pass 1 / global init */
            break;
        default:
            codegen_decl(ctx, decl);
            break;
        }
    }

    /* Inject calls to __ls_ffi_init and __ls_global_init at the start of main() */
    {
        ...
        LLVMValueRef global_init = LLVMGetNamedFunction(ctx->module, "__ls_global_init");
        ...
        if (main_fn && LLVMCountBasicBlocks(main_fn) > 0) {
            ...
            if (first) {
                ...
                if (global_init)
                    LLVMBuildCall2(tmp, init_type, global_init, NULL, 0, "");
                LLVMDisposeBuilder(tmp);
            }
        }
    }
