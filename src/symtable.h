...
typedef struct Symbol {
    ...
    bool is_comptime_const; /* docs/plan_comptime_consteval.md: a `comptime <scalar> X = ...`
                               constant — immutable, and folded into a literal at every use
                               site (so codegen never sees the name, only the value). */
    bool ct_is_float;       /* when is_comptime_const: true → ct_f holds the value (f32/f64),
                               false → ct_i holds it (int/sized-int/bool/char). */
    long long ct_i;         /* integer / bool / char comptime value */
    double    ct_f;         /* floating-point comptime value */
} Symbol;
