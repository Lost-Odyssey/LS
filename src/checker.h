...
typedef struct Checker {
    ...
    /* Generic instantiation recursion depth. checker_instantiate_struct <->
       resolve_type_node_with_substitution <-> instantiate_template are mutually
       recursive, and a self-referential template (struct Rec(T) { Rec(Rec(T)) x })
       expands without bound -- that used to overflow the stack and kill the process
       (0xC00000FD) with no diagnostic at all. Mirrors parser.c's p->depth /
       LS_MAX_PARSE_DEPTH; the parser guard cannot cover this because the nesting is
       generated by instantiation, not written in the source. Zeroed by the
       memset in the Checker setup. */
    int inst_depth;
