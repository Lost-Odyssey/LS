...
static const Keyword keywords[] = {
    ...
    {"array",    5, TOKEN_ARRAY},
    {"as",       2, TOKEN_AS},
    {"bf16",     4, TOKEN_TYPE_BF16},
    {"bool",     4, TOKEN_TYPE_BOOL},
    {"break",    5, TOKEN_BREAK},
    {"char",     4, TOKEN_TYPE_CHAR},
    {"comptime", 8, TOKEN_COMPTIME},
    {"continue", 8, TOKEN_CONTINUE},
    {"def",      3, TOKEN_FN},        /* de-Rust: replaces Rust's `fn` (TOKEN_FN kept) */
    {"do",       2, TOKEN_DO},
    {"else",     4, TOKEN_ELSE},
    {"end",      3, TOKEN_END},
    {"enum",     4, TOKEN_ENUM},
    {"extern",   6, TOKEN_EXTERN},
    {"f16",      3, TOKEN_TYPE_F16},
    ...
    {"interface",9, TOKEN_TRAIT},     /* de-Rust: replaces Rust's `trait` (TOKEN_TRAIT kept) */
    {"lib",      3, TOKEN_TYPE_LIB},
    {"load",     4, TOKEN_LOAD},
    {"match",    5, TOKEN_MATCH},
    {"methods",  7, TOKEN_IMPL},      /* de-Rust: replaces Rust's `impl` (TOKEN_IMPL kept) */
...
static Token scanner_next_inner(Scanner *s) {
    ...
    case '@':
        ...
        /* @print — the print intrinsic, a dedicated token like @time/@bench (the
           ONLY spelling; bare print is retired). docs/plan_print_sink.md Stage F. */
        if (strncmp(s->current, "print", 5) == 0 &&
            !isalnum((unsigned char)s->current[5]) && s->current[5] != '_') {
            s->current += 5; s->column += 5;
        ...
        return error_token(s, "unknown annotation (expected @name, e.g. @derive)");
    }
...
const char *token_type_name(TokenType type) {
    ...
    case TOKEN_TYPE_BF16:     return "TYPE_BF16";
