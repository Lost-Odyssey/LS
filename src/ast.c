...
/* P5-2 prelude (docs/plan_p5_remove_builtin_string.md §5): prepend `import std.core.str`
   to a ROOT program so the default literal type (Str) resolves without every file
   spelling the import. No-op when the node isn't a program or std.core.str is already
   imported. Root files only — modules pull their own imports (injecting into
   std.c would create the cycle std.core.str→std.c→std.core.str); modules inside
   std.core.str's dependency cone resolve Str through the checker's registry fallback. */
void ast_inject_std_str_import(AstNode *program) {
    ...
    imp->as.import_decl.path = ast_strdup("std.core.str");
...
void ast_free(AstNode *node) {
    ...
    case AST_IDENT:
        ...
        for (int i = 0; i < node->as.ident.type_arg_count; i++)
            type_node_free(node->as.ident.type_args[i]);
    ...
    case AST_SIZEOF:
        ...
        break;
    case AST_TYPENAME:
        type_node_free(node->as.typename_expr.type_node);
        break;
    case AST_TRY:
    ...
    case AST_COMPTIME_FOR:
        free(node->as.comptime_for.var);
        type_node_free(node->as.comptime_for.over_type);
        ast_free(node->as.comptime_for.body);
        break;
    case AST_COMPTIME_IF:
        ast_free(node->as.comptime_if.cond);
        ast_free(node->as.comptime_if.then_block);
        ast_free(node->as.comptime_if.else_block);
        break;
    case AST_COMPTIME_FIELD:
        ast_free(node->as.comptime_field.object);
        free(node->as.comptime_field.handle);
        break;
    case AST_COMPTIME_MATCH:
        ast_free(node->as.comptime_match.subject);
        free(node->as.comptime_match.handle);
        free(node->as.comptime_match.binder);
        ast_free(node->as.comptime_match.body);
        break;
    case AST_COMPTIME_CONST:
        type_node_free(node->as.comptime_const.decl_type);
        free(node->as.comptime_const.name);
        ast_free(node->as.comptime_const.value);
        break;
    case AST_COMPTIME_BLOCK:
        ast_free(node->as.comptime_block.block);
        break;
    case AST_BLOCK:
    ...
    case AST_STRUCT_DECL:
        ...
        free(node->as.struct_decl.field_private);
        for (int i = 0; i < node->as.struct_decl.derive_count; i++)
            free(node->as.struct_decl.derives[i]);
        free(node->as.struct_decl.derives);
        break;
    case AST_ENUM_DECL:
        ...
        for (int i = 0; i < node->as.enum_decl.derive_count; i++)
            free(node->as.enum_decl.derives[i]);
        free(node->as.enum_decl.derives);
        break;
    case AST_IMPL_DECL:
...
const char *ast_kind_name(AstNodeType kind) {
    ...
    case AST_COMPTIME_FOR:   return "COMPTIME_FOR";
    case AST_COMPTIME_IF:    return "COMPTIME_IF";
    case AST_COMPTIME_FIELD: return "COMPTIME_FIELD";
    case AST_COMPTIME_MATCH: return "COMPTIME_MATCH";
    case AST_COMPTIME_CONST: return "COMPTIME_CONST";
    case AST_COMPTIME_BLOCK: return "COMPTIME_BLOCK";
...
AstNode *ast_clone_deep(const AstNode *src) {
    ...
    *n = *src;                  /* shallow copy first */
    ...
    case AST_IDENT:
        ...
        n->as.ident.type_arg_count = src->as.ident.type_arg_count;
        if (src->as.ident.type_arg_count > 0) {
            n->as.ident.type_args = (TypeNode **)malloc_safe(
                (size_t)src->as.ident.type_arg_count * sizeof(TypeNode *));
            for (int i = 0; i < src->as.ident.type_arg_count; i++)
                n->as.ident.type_args[i] = type_node_clone(src->as.ident.type_args[i]);
        }
        break;
    case AST_UNARY:
    ...
    case AST_VAR_DECL:
        ...
        n->as.var_decl.is_repl_extern = src->as.var_decl.is_repl_extern;
        break;
    case AST_ASSIGN:
    ...
    case AST_FOR_C:
        ...
        break;
    case AST_COMPTIME_FOR:
        n->as.comptime_for.var          = ast_strdup(src->as.comptime_for.var);
        n->as.comptime_for.over_type    = type_node_clone(src->as.comptime_for.over_type);
        n->as.comptime_for.body         = ast_clone_deep(src->as.comptime_for.body);
        n->as.comptime_for.over_variants = src->as.comptime_for.over_variants;
        break;
    case AST_COMPTIME_IF:
        n->as.comptime_if.cond       = ast_clone_deep(src->as.comptime_if.cond);
        n->as.comptime_if.then_block = ast_clone_deep(src->as.comptime_if.then_block);
        n->as.comptime_if.else_block = ast_clone_deep(src->as.comptime_if.else_block);
        break;
    case AST_COMPTIME_FIELD:
        n->as.comptime_field.object = ast_clone_deep(src->as.comptime_field.object);
        n->as.comptime_field.handle = ast_strdup(src->as.comptime_field.handle);
        break;
    case AST_COMPTIME_MATCH:
        n->as.comptime_match.subject = ast_clone_deep(src->as.comptime_match.subject);
        n->as.comptime_match.handle  = ast_strdup(src->as.comptime_match.handle);
        n->as.comptime_match.binder  = ast_strdup(src->as.comptime_match.binder);
        n->as.comptime_match.body    = ast_clone_deep(src->as.comptime_match.body);
        break;
    case AST_COMPTIME_CONST:
        n->as.comptime_const.decl_type = type_node_clone(src->as.comptime_const.decl_type);
        n->as.comptime_const.name      = ast_strdup(src->as.comptime_const.name);
        n->as.comptime_const.value     = ast_clone_deep(src->as.comptime_const.value);
        n->as.comptime_const.is_global = src->as.comptime_const.is_global;
        break;
    case AST_COMPTIME_BLOCK:
        n->as.comptime_block.block = ast_clone_deep(src->as.comptime_block.block);
        break;
    case AST_CAST:
    ...
    case AST_SIZEOF:
        ...
        break;
    case AST_TYPENAME:
        n->as.typename_expr.type_node  = type_node_clone(src->as.typename_expr.type_node);
        n->as.typename_expr.named_type = NULL; /* re-resolved per instantiation */
        break;
    case AST_TRY:
...
void ast_print(AstNode *node, int indent) {
    ...
    case AST_FOR_C:
        ...
        break;
    case AST_COMPTIME_FOR:
        printf("COMPTIME_FOR(%s in fields(", node->as.comptime_for.var);
        type_node_print(node->as.comptime_for.over_type);
        printf("))\n");
        print_indent(indent + 1);
        printf("body:\n");
        ast_print(node->as.comptime_for.body, indent + 2);
        break;
    case AST_COMPTIME_IF:
        printf("COMPTIME_IF\n");
        print_indent(indent + 1);
        printf("cond:\n");
        ast_print(node->as.comptime_if.cond, indent + 2);
        print_indent(indent + 1);
        printf("then:\n");
        ast_print(node->as.comptime_if.then_block, indent + 2);
        if (node->as.comptime_if.else_block) {
            print_indent(indent + 1);
            printf("else:\n");
            ast_print(node->as.comptime_if.else_block, indent + 2);
        }
        break;
    case AST_COMPTIME_FIELD:
        printf("COMPTIME_FIELD(.(%s))\n", node->as.comptime_field.handle);
        ast_print(node->as.comptime_field.object, indent + 1);
        break;
    case AST_COMPTIME_MATCH:
        printf("COMPTIME_MATCH(%s(%s))\n", node->as.comptime_match.handle,
               node->as.comptime_match.binder ? node->as.comptime_match.binder : "");
        ast_print(node->as.comptime_match.subject, indent + 1);
        ast_print(node->as.comptime_match.body, indent + 1);
        break;
    case AST_COMPTIME_CONST:
        printf("COMPTIME_CONST(%s)\n", node->as.comptime_const.name);
        print_indent(indent + 1);
        printf("type: ");
        type_node_print(node->as.comptime_const.decl_type);
        printf("\n");
        print_indent(indent + 1);
        printf("value:\n");
        ast_print(node->as.comptime_const.value, indent + 2);
        break;
    case AST_COMPTIME_BLOCK:
        printf("COMPTIME_BLOCK\n");
        ast_print(node->as.comptime_block.block, indent + 1);
        break;
    case AST_BLOCK:
