...
**当前测试**：ctest 151/151（分支 `feat/rawvec`：纯 LS 自管裸内存容器 `std.rawvec` RawVec(T)，M0~M2 + owned-param/move-into-container ABI（string push 追平 vec）+ realloc/sizeof/ptr-index 基础设施）
