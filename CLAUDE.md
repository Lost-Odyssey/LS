...
**当前测试**：ctest 186/186（分支 `feat/map-index-protocol`：Map 索引协议 `m[k]`/`m[k]=v` 缺键 panic，对齐 Vec `v[i]` 三层模型；纯 LS std/map.ls，编译器零改动。前序分支 `feat/runtime-primitives-stdc` 已 merge：内存原语移入 std.c + Vec mutator 越界检查（#4）+ sort_by 稳定归并排序（#2）；#3 clone-on-read 记为 known_limitations L-006）
