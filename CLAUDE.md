...
**当前测试**：ctest 188/188（分支 `feat/match-result-ownership`：**L-013 match 结果所有权整改**——match 作 owned-rvalue，臂封装 + string 单结果临时登记 + has_drop 臂内 clone 归一）。前序 `feat/opt-combinators-c1` 已 merge：Option/Result 组合子 **C1**（unwrap/expect/unwrap_or/is_*?）+ **C2a**（ok/err/ok_or 互转）；更前 `feat/map-index-protocol`：Map 索引协议）
