...
| — | **规范路径 std.c 内存原语 + std.vec/map 迁移**（2026-06-10，[docs/plan_runtime_primitives.md](docs/plan_runtime_primitives.md)）：`std.c.{malloc,realloc,free,abort}` 现可按**完整规范路径**够到（任何位置，含**泛型方法体**——它在消费方实例化、局部 `import std.c as c` 别名已被销毁,故别名不可用而完整路径可达）。实现**务实收窄**：checker `match_stdc_prim` + codegen `cg_match_stdc_prim` 各做字面结构匹配 `AST_FIELD(AST_FIELD(IDENT"std","c"),{prim})`,复用既有 bare-name intrinsic lowering(malloc/realloc→CRT+size SExt 到 i64,free→drop+free,abort→`__ls_proc_exit(1)`),**无需通用 registry 路径解析/跨模块发射/std.c 真发射**;通用 `mod.fn` 规范路径留 B-full。`std/vec.ls`(realloc/free/abort)+`std/map.ls`(realloc/free)迁到 `std.c.*`,交付 bugs/27_vec #1。**裸 builtin 保留**——施工审计发现 malloc/free/realloc/abort 是**面向用户的裸堆原语**(~15 测试样本直接用,非 std 内部),删除属破坏性语言表面决策,暂留待定(见 plan §9 选项 A/B/C)。`test_stdc_prim`(JIT+AOT+memcheck,直接+泛型体两路)。ctest 183/183 | ✅ |

**当前测试**：ctest 183/183（分支 `feat/runtime-primitives-stdc`：新增 **规范路径 `std.c.{malloc,realloc,free,abort}`** + std.vec/map 迁移；裸 builtin 保留待定，见上记录）
