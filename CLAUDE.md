...
**当前测试**：ctest 187/187（分支 `feat/opt-combinators-c1`：Option/Result 组合子 **C1**（unwrap/expect/unwrap_or/is_*?，复用 force-unwrap+match）+ **C2a**（ok/err/ok_or 互转，bare-ctor hint 消歧）。前序 `feat/map-index-protocol` 已 merge：Map 索引协议；更前 `feat/runtime-primitives-stdc`：内存原语入 std.c + Vec #4 越界 + #2 归并排序）
