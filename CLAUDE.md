...
| — | **string→Str P5-4 S-3+S-4：builtin string 全链路拆除收官**（2026-06-12，commit 1e24c3a/e96492e/b692384/47dfd44/6934416/f119e69，[docs/plan_p5_remove_builtin_string.md](docs/plan_p5_remove_builtin_string.md) §8/§10）：**S-3 codegen 四刀**（每刀全量 ctest 192/192）——**S-3a** 删死 string 生产者/方法发射 ~1.9k 行（codegen_string_method 1372 行+派发、to_string/from_int/from_float/take_buffer、字面量/f-string LsString fallback、binary 拼接+六 strcmp 块、Step-11 string case；唯一活路径修复＝checker AST_FORCE_UNWRAP 补 check `expect("msg")` 消息表达式，原靠未定型字面量 fallback 兜住）；**S-3b** 删 call-ABI/B 桥发射/消费侧决策树 ~670 行（三桥函数+六消费点、LS_CAP_BORROWED 实参标记、C-ABI fixup、BF-025、var_decl/assign/return/field/array/*ptr 的 TYPE_STRING 树；AST_RETURN borrowed_heap 的 struct/enum 路径不动＝保第 14 修复）；**S-3c** 红线区两刀（按 [docs/match_codegen_guide.md](docs/match_codegen_guide.md) 逐站点核对，6 臂体存储点同步）——c-1 删 match/try/closure-env/容器字面量 string 死臂 + cg_register_result_temp 整删（guide §3.2 string 半边），c-2 **temp_string 平行表整体拆除**：slots/count/cap 字段+temp_drop_marks 删（marks＝temp_string_count 快照，S-2 后恒 0 已退化为全量冲洗，删除＝行为保持），cg_flush_temps 改无参版，own_tail/encapsulate 去 str_mark 形参，cg_store_owned 删 STRING 分支——**统一路径自此唯一：Str/struct/enum 全走 temp_drop（L-013 病根落点收官）**；**S-3d** 底座清账（emit_string_*/ls_string_* 家族、__ls_str_split·join·lines 651 行、scope/struct/enum/global/closure 全部 string drop·clone 分支、ast.h 三个 B 桥 coerce flag、common.h LS_MIN_STR_CAP+LS_CAP_*）。**S-4 types**：删 TYPE_STRING 枚举本体+PRIM_STRING+type_string()+checker/codegen/test 全部 case。**§8 验收达成**：`grep TYPE_STRING|TOKEN_TYPE_STRING|temp_string_|emit_string_|ls_string_type|LS_CAP_BORROWED src/ tests/*.c` **零命中**——builtin string 自 scanner→checker→codegen→types 拆除完毕，**Str（纯 LS std.str）为唯一字符串类型**。**性能红线**：benchmarks/alloc 迁 Str/Map 后 pre-S3 vs HEAD A/B（git worktree 构建 fff64c9）JIT/AOT 均等价（噪声内），memcheck 0/0/0。ctest 192/192 | ✅ |

| — | **P5-4 遗留疑似 bug 集中清账（A/B/C/D-1）**（2026-06-12，commit a3342b7~3a5f85b，账本 [docs/bugs_deferred_p5_4.md](docs/bugs_deferred_p5_4.md)）：移植期甄别出的挂账问题逐个闭环，每项独立提交、全程 ctest 全绿。**A 四个疑似真编译器 bug**：**A-1** 函数体内嵌套 fn/struct/impl 定义 → codegen "no terminator"——选「不支持」，`parse_block` 解析完每条语句按 kind 清晰拒绝（fn/struct/enum/impl/trait/module），不再下沉 codegen（负向 smoke `nested_fn_decl_reject`）；**A-2** 显式 `.__drop()` 调用 → JIT "Symbols not found"——原 `checker_warning` 升级为 `checker_error`（任何位置的 `.__drop()` 方法调用，本就双释 footgun），删两孤儿样本 member_drop_call/correct、test_codegen 的 test_struct_drop_nested 改依赖自动嵌套 drop（负向 smoke `explicit_drop_reject`）；**A-3** struct 字段前向引用 → 段错——根因 `check_struct_decl` 的 has_drop 扫描 `ft->kind` deref NULL（前向引用字段 `resolve_type_node` 返 NULL），加 `if(ft==NULL)continue` 优雅 rc=1 退出（前向引用本身不支持，负向 smoke `forward_ref_struct_reject` 断言 rc==1 非崩溃）；**A-4**（最深）match 链式拼接接收者 spill 崩溃——S-4 后形态从 0/127 间歇变稳定 rc=127，最小化与 io 无关＝纯 `Result(Str,Str)` 连续 ≥3 match 块 + 臂内 `if` 体 3 段拼接 `"lit"+s+"lit"`；根因＝链式 `+` 内层结果 spill 到 `tmp.rval.self`（codegen_addr_of ~4256）并登记 temp_drop，但**初始化 store 在 `if.then`（未走分支），drop 在落空路径 `match.end` 发射 → free 未初始化栈 alloca**；修复＝新 helper `cg_entry_alloca_zeroed` 在 entry block 零初始化该 spill（与 match `result_alloca` 零初始化防御同源，**未改 match codegen**），自包含回归 `match_concat_temp_test`。**B 语言可用性**：B-1（`v[i]!="lit"` 误标 moved）、B-2（组合子 arg 不 plumb expected）经当前 HEAD 复核**翻默认 P5-2 后自动消解**（字面量天然 Str）；**B-3** 用户 `impl` 导入 struct（如主文件 `impl Str`）方法符号发 bare `Str.shout` 但 dispatch 经 llvm_name 解析 `std_str__Str.shout` → "Symbols not found"——`codegen_impl_decl` 在 struct_name 仍 bare 且非 builtin impl 时按 bare 名查注册表 `llvm_name` 并采用（镜像 dispatch），回归 `impl_imported_struct_test`。**C 测试基建**：C-1 注册孤儿 `test_fs.cmake`（fs_test 从未进 ctest）+ RESOURCE_LOCK；C-2 假绿审计——给 17 个「断言 ALL PASS 却不否决 FAIL 且无 memcheck」的驱动机械插入 FAIL 否决块（纯加安全网）；C-3 `docs/build.md` 新增「AOT Defender flake 与缓解」节（Add-MpPreference 排除项治本 + `--repeat until-pass:2` 兜底）。**D-1** struct auto-print 识别 Str 字段——`codegen_print_struct_value` 在 `cg_type_is_str` 字段时改调 `cg_print_str_value`（`%.*s` + 引号）先于通用 struct 递归，`print(含 Str 字段 struct)` 从 `Str{data=指针...}` 改为 `name="text"`，回归 `test_struct_print_str`（精确格式 + 拒绝 `Str{data=` + memcheck）。**D-2**（L-010 REPL 跨 snippet 析构段错）评估后**不做**（REPL/JIT 模块发射重构、独立较大、影响 REPL-only 边界，已记账）。ctest 199/199 | ✅ |

| — | **std.str 性能 P0：contains/replace 运行时加速**（2026-06-12，[benchmarks/strbench/strbench_analysis.md](benchmarks/strbench/strbench_analysis.md)）：新增运行时辅助 **`__ls_str_find(hay,hlen,needle,nlen,start)`**（[runtime/builtins.c](runtime/builtins.c)，memchr 定位首字节+memcmp 校验，CRT SIMD 加速；裸 ptr+len 无 NUL 要求 → `&self` 只读借用安全，**修正旧分析「改调 strstr」的建议**＝strstr 要求两端 NUL 破坏 `find(&self,&Str)` 签名+嵌入 NUL 截断）。接线 4 处：[src/jit.c](src/jit.c) REG(90)/`pairs[91]`、[std/c.ls](std/c.ls) extern fn、[std/str.ls](std/str.ls) 改 `import std.c as c`（通用 `std.c.X` 路径仅 malloc/realloc/free/abort 字面匹配，任意 extern 须走别名 `c.X`，同 regex.ls）后 `c.__ls_str_find(...)`。**str.ls 改写**：`find`/`count` 转调运行时（O(n×m) 朴素循环→memchr+memcmp，`contains?` 经 find 自动受益）；**`replace` 加单字节等长 fast-path**（`old.len==1 && rep.len==1`：clone self 后 memchr-scan 原地改字节不重建），通用路径用 `__ls_str_find` 定位下一匹配。实测 AOT n=1M（checksum 208,246,494 不变）：contains 38,261→17,927 µs（2.1×，残差是每迭代 LS→C 调用边界）、**replace 427,461→108,424 µs（3.9×，已低于 C++ 124,263）**。upper/split/substr 未动。ctest 199/199 | ✅ |

| — | **std.str 性能 P0 算法升级：自适应 BMH/Sunday**（2026-06-12，应用户「能否上 Boyer-Moore-Horspool/fastsearch」之问，[benchmarks/strbench/strbench_analysis.md](benchmarks/strbench/strbench_analysis.md)）：`__ls_str_find` 从「memchr+memcmp 一招」改为**按输入长度自适应**（CPython fastsearch / glibc 同思路，无单一算法处处最优）——`nlen==1`→memchr；`hlen<256`（短）→memchr+memcmp **不建跳表**（小串搜百万次时建 256 项坏字符表是净负担，strbench 走此路零回归）；长文本→**Sunday Quick-Search 坏字符跳表**（memchr 仍 SIMD 定位首字节，校验失败按 Sunday 移位跳跃消除 O(n×m) 退化）。**诚实结论**：memchr+memcmp 本就是 SIMD 工业级（非暴力），纯算法微基准（`bmh_bench.c`，隔离 FFI，200KB 随机 prose）Sunday 仅快 ~16%（1.16-1.17×）——真实但不戏剧性，底子已强；strbench contains（43B 短串）不受影响、残差仍是 FFI 调用开销。回归测试 str_perf_methods 扩长文本（>256B）用例验证 Sunday 路径与短路径结果一致。新增独立 C 微基准 `benchmarks/strbench/bmh_bench.c`（naive vs sunday 对拍，Case A 病态例被编译器循环外提故无效、信号取 B/C）。ctest 200/200 | ✅ |

| — | **std.str 性能 P1+P2：拷贝 memcpy 化 + split_view 零拷贝**（2026-06-12，[benchmarks/strbench/strbench_analysis.md](benchmarks/strbench/strbench_analysis.md)）：**P1** 新增运行时 **`__ls_bytecopy(dst,doff,src,soff,n)`**（[runtime/builtins.c](runtime/builtins.c)，单次 memcpy；带字节偏移因 **LS 无指针算术**实测 `(p+1)` 报 type error，调用方无法自取 `&buf[off]`）；[src/jit.c](src/jit.c) REG(91)/`pairs[92]`、[std/c.ls](std/c.ls) extern；[std/str.ls](std/str.ls) 把 substr/concat/__clone/reserve/push_str/`+`/repeat 逐字节拷贝循环全换 `c.__ls_bytecopy(...)`。**结论：本微基准中性**——substr ~58ms/split ~937ms 与 P0 持平（基准用小串 43B/5B/~4B，AOT 已向量化短循环，换 memcpy 调用开销相抵）；**价值在大缓冲 KB+ 与 JIT（无向量化）**+ 统一拷贝路径，零回归。substr 逼近 C++ 10ms 必须 SSO（memcpy 去不掉 5B malloc）。**P2** 新增 `struct StrSlice{int off;int len}`（POD 非 has_drop）+ `split_view(&Str)->Vec(StrSlice)`（`__ls_str_find` 定位分隔符，产字节偏移视图非 owned Str 段——K 段从 K 次 malloc 降到 0）+ `slice_str(StrSlice)->Str` 按需物化；`split` 语义/边界不变。实测 AOT n=1M：**split 937,000→split_view 300,000 µs（3.1×）**，逼近 Python split（352ms）；无生命期系统下这是 split 极限（Rust `&str` 切片需借用返回，LS 用 StrSlice 偏移对等价「推迟拷贝」）。全程 checksum 208,246,494 不变。回归 `tests/samples/str_perf_methods.ls`（test_str_perf_methods，find/contains/count/replace 单+多+空/concat/+/repeat/substr/push_str/split/split_view/slice_str/边界，JIT+AOT+memcheck 0/0/0）。ctest 200/200 | ✅ |

| — | **`Vec.get(i) → Option(T)` 破坏性重构（三层访问模型收官 + 所有权全仓验收）**（2026-06-12，分支 `feat/get-option`，[docs/plan_container_access_safety.md](docs/plan_container_access_safety.md) §5.6）：`get` 从「checked panic 返 T」改为在界 `Some(克隆)` / 越界 `None`——三层模型补全（`v[i]`=panic / `get`=可恢复 / `get!`=unchecked），与 Map.get/pop/first/last/find 对齐；C1/C2a 组合子+`try`+后缀 `!` 已就位故开箱即用（`v.get(i).unwrap_or(d)`/`try v.get(i).ok_or(e)`/`v.get(i)!`）。`__index` 改自带检查+abort 不再转调 get（保 `-> T` 不变量，`v[i].foo()` 链零影响）。**实际破坏面远小于旧估 253 处**（那是含 Map/env 已-Option 站点的全仓 `.get(` 总数）：std ~83 处（html 26/md 50 全是 `for i in 0..len` 在界循环读→机械 `get!`，**borrow-match 绑定 Vec 不能用 `[i]`**（plan_std_map §13）故 `get!` 是这些站点唯一选择；stack pop/peek None 臂 fall-through 改 `data[0]` 保 panic 兜底；ring 2 处 `get!`；json 2 处是 Map.get 零改动）+ 样本侧 26 个失败测试 ~95 处 + 3 个 ctest 外遗留（vecbench/孤儿 vec_get_test/phase_h_repro），**全部编译期响亮 type error、零运行期失败**（同 P5-2 翻转性质）。**所有权全仓验收达成**：每个 get 站点都成为 owned-rvalue `Option(T)` 主体经 match/组合子消费，L-012/L-013/OPT-001/M5-004 链路数百站点 × JIT+AOT+memcheck 锻打**零新所有权 bug**（L-013 记账「get→Option 必撞 temp_drop 双释」未复现＝整改验收通过）。新回归 `test_vec_get_option`（POD/owned Str/has_drop struct/嵌套 Vec × match 绑定/unwrap_or owned fallback/is_none?/`get(i)!` 强解 move/`try`+`ok_or` 传播，0/0/0）；`vec_oob_test` 重写为三层模型验收（get Some/None 双侧 + 强解 + panic 负向不变）。实测踩点：显式 `&局部` 实参报 `*Vec` 不匹配（§13 既有挂账），auto-borrow 规避。ctest 201/201 | ✅ |

| — | **plan_std_map §13 挂账双修**（2026-06-12，分支 `fix/s13-borrow-gaps`）：① **borrow-match binder Vec 下标 `[i]`**——根因＝导入泛型实例（如 `Vec(std_str__Str)` 由 std.json 的 checker 实例化盖章）的 impl 方法未在消费方 checker 注册：方法调用路径早有 VR-LIM-018 回退（`ensure_generic_struct_impls_local` 按需注册），但 `__index`/`__index_set` 协议门口是裸 `find_method` 探测 → 未命中报「cannot index non-array type 'Vec(...)'」。修＝新增 `find_method_ensured`（未命中且 `generic_base` 非空 → ensure 后重查）替换两门口（[src/checker.c](src/checker.c) AST_INDEX/AST_ASSIGN）。本地实例化 Vec 从不受影响（var_decl 经 resolve_type_node 已注册），只有「类型仅经模块盖章、从未本地解析」的 borrow-match binder 站点踩中——故 html/md 循环读站点的 `get!` 不必回改（unchecked 本就是性能选择），`phase_h_repro` 的 `get!(0)` 规避改回 `[0]`。② **显式 `&局部` 实参传只读 `&T` 参数**——`&x` 一元统一产裸 `*T` 与 TYPE_REFERENCE 形参不可赋值（报 `expected '&Vec(int)', got '*Vec(int)'`），只有 auto-borrow 裸传能走通。修＝checker 实参循环对「只读 `&T` 形参 + `AST_UNARY(&)+AST_IDENT` 实参」**剥 `&` 壳**与裸传同路（复用成熟 auto-borrow checker+codegen 机制，零新 ABI 代码）；`&!x` 是独立 AST_MUT_BORROW 不受影响，`&x` 冒充 `&!T` 仍编译拒（负向 `s13_amp_mutref_reject`）；非 IDENT 操作数（`&s.field`）保持裸指针语义不剥。`test_s13_borrow_gaps`（本地单/双载荷 + 导入实例 binder 下标 + `&Vec`/`&Str`/`&Enum` 显式借用与源存活，JIT+AOT+memcheck 0/0/0 + 负向）。§13 仍挂账：`Block(&!T)`。ctest 202/202 | ✅ |

| — | **Block(&!T) 可写借用块参数（§13 收官）**（2026-06-12，分支 `fix/s13-borrow-gaps`）：排查发现现状比挂账记录更糟——**编译通过 + 运行时堆损坏**（0xC0000374），静默 unsound。M5-002 修 `Block(&T)` 的三处中两处早已覆盖 `&!`（closure 体绑定 `pt->is_mut→is_mut_borrow` 指针直绑；checker 负向①只读体内调 `&!self` 拒、②裸 `v` 传 `&!T` 拒均生效），唯一断点＝**调用点 `codegen_block_call` TYPE_REFERENCE 分支只认 AST_IDENT**：`&!v`（AST_MUT_BORROW）落进 rvalue 物化兜底，把指针当 pointee 值存进临时再借出 → 闭包对垃圾内存写。修＝该分支对 AST_MUT_BORROW（或实参 resolved_type 已是引用）直传指针（[src/codegen.c](src/codegen.c)）。`test_block_mutref`（Vec/Str/Map 载体、`fn(&!T 参数)` 内转发 `f(&!v)`、多次调用、混合 POD+`&!`、只读 `&v`/裸 `v` 对照（§13② 剥壳同样作用于 Block 调用）、源存活，JIT+AOT+memcheck 0/0/0 + 两负向）。**plan_std_map §13 挂账全部清零**。ctest 203/203 | ✅ |

| — | **Option/Result 组合子 C2b（闭包组合子收官）**（2026-06-12，[docs/plan_container_access_safety.md](docs/plan_container_access_safety.md) §5.5.1）：补齐 `map(U)`/`and_then(U)`/`map_err(F)`/`unwrap_or_else`，仍纯 checker lower（**零 codegen**），复用 C1/C2a 的 `lower_opt_combinator` 框架。**两大设计**：① **显式结果类型参 U**（`opt.map(U)(\|x\| ..)`），对齐既有方法级泛型约定（`vec.map(int)(...)`），lower 读 `node->as.call.type_args` 构造 `Option(U)`/`Result(U,E)` 压为期望类型让臂体 bare ctor 消歧（同 C2a）；`unwrap_or_else` 无类型参（结果即 T）。② **内联闭包体，不调用闭包**——绕开「闭包作 callee 无 Block 期望类型」的推断坑（§5.3 路 B 核心限制）：把闭包参名直接当 match 臂 binder，闭包体 `{return E}` 块的尾 return 原地改写为 expr-stmt 内联进臂体，**无闭包值/env/捕获机制**（被捕获变量作普通读在 enclosing scope 解析），故零新所有权代码复用 match drop/move。脱糖：map=`Some(x)=>Some(BODY) None=>None`（Result 映射 Ok、Err 重建透传）、and_then=`Some(x)=>BODY`（BODY 已产 Option/Result）、map_err=`Ok(v$)=>Ok(v$) Err(e)=>Err(BODY)`、unwrap_or_else=`Some(v$)=>v$ None=>BODY`。**附带通用修复**：match 检查 **subject** 时清除 `c->expected_type`（subject 类型自洽，绝不应被 match 期望结果类型胁迫；否则链式 `Some(3).map(Str)(..)` 脱糖后 subject `Some(3)` 误判 `expected 'Str' got 'int'`）。**限制**：闭包须字面量 `\|x\| expr`、无早 `return`，均编译期清晰报错。**正交既有 bug 记账**（git stash 验证 baseline 复现，非 C2b 引入）：`Result(Str,Str).unwrap_or` 接收者为 Ok 时泄漏 owned 成功载荷（纯 C1 owned 条件移动边界，留 L-013 后续）→ 故 C2b 测试 owned 结果一律 `match` 消费规避。`test_opt_combinator` 正向 +18 项（四组合子 × Option/Result + owned Str move-out/透传 + `v.get(i).map(..)` 头号链式，JIT+AOT+memcheck 0/0/0）+ 负向 `c2b_no_typearg`/`c2b_map_err_option`。ctest 203/203 | ✅ |
| — | **std.tensor 堆张量（NumPy/AI）**（2026-06-13，[docs/plan_ndarray_stdlib.md](docs/plan_ndarray_stdlib.md) §-1，分支 `feat/std-tensor`）：纯 LS `Tensor(T)`（`*T data + Vec(int) shape/strides + size`，move/拷贝语义，零内建 array 依赖）。阶段 0 地基（堆 `*T`+偏移+as_ptr+C memcpy）→ 1 泛型 Tensor（`{}`+init idiom + flat 三层 get/set/get!/set! + reshape + for-in + copy）→ 2a 多下标 `t[i,j,k]`（arity 派发协议，parser 收逗号下标→checker 改写 `__index{N}`，零 codegen）→ 3a 数值核心（elementwise/reduction/matmul/transpose/relu）→ 3b NumPy broadcasting + 按轴 reduction + float 激活 → 3c std.rand（xorshift64\* PRNG）+ 随机初始化 + tanh + mean/max_axis → 3d 安全拷贝切片 row/col/slice + argmax_rows + 逐元素 + mse → 一行泛型构造 zeros/full/from_vec + 端到端 MLP demo。**编译器改进**：ambient builtin 模块解析（泛型方法体内 `math.X` 无需调用方 import）。视图 TensorView 评估后推迟（LS 无生命期=悬垂 footgun，拷贝切片覆盖 90%）。性能走 BLAS/SIMD/FFI（后续）。各阶段 JIT+AOT+memcheck 0/0/0 | ✅ |
| — | **std.complex + std.fft（复数 + FFT 库）**（2026-06-13，[docs/plan_fft_stdlib.md](docs/plan_fft_stdlib.md)，分支 `feat/std-tensor`）：**编译器 3 新特性**——① 静态 trait 方法（`trait Zero { static fn zero()->Self }`，解除 trait-impl 静态禁令）② `T.zero()` 在泛型类型参上派发（静态调用认 type-alias + 盖 resolved_type，codegen 零改动）③ **泛型 trait impl** `impl(T) Add for Complex(T)`（parser 重构 `(T)` 移到 trait 检测前 + 折叠机制：方法折进 struct 的 inherent 泛型 impl_node 复用既有单态化/codegen/派发 + satisfies_trait 泛型基名匹配；坑：折叠须转移所有权避 ast_free 双释）。**std.num** `trait Zero`+int/i64/f64；**std.complex** 泛型 `Complex(T){T re;T im}`+运算符 `+-*/`+conj/norm/abs/arg/exp/from_polar+自由函数 c/real/from_polar（A2 泛型类型直接静态调用 `Complex(f64).zero()` parser 歧义不修，workaround 充足）。**std.fft**（非泛型，全 Complex(f64)）：1D radix-2 → Bluestein 任意 N（含大素数）→ rfft/irfft + DCT-II/III → ND fft2/fftn（Tensor(Complex) multi-pass gather/scatter）。**性能**：Makhoul DCT(O(N²)→O(N log N))、打包 rfft(偶 N 2×)、递归 mixed-radix(smooth N 省 Bluestein)。**编译器修复**：泛型自由函数嵌套类型参 mangle（`zeros(Complex(f64))`）；**根治 `if/while (expr)<op>y {` 老解析坑**（std.map M-0 起，删 has_paren 特判）。tensor.ls 全部 `0 as T`→`T.zero()`。各阶段对拍朴素 DFT/DCT + round-trip，JIT+AOT+memcheck 0/0/0。唯一剩余=FFI pocketfft/KISS(BSD) 极致性能（按需） | ✅ |

| — | **alloc 性能：纯 LS Map/Str 化后的回退诊断 + 第二轮优化**（2026-06-13，[benchmarks/alloc/alloc_analysis.md](benchmarks/alloc/alloc_analysis.md)）：重测发现 M-6（拆 builtin map）+ P5-4（拆 builtin string）后 alloc **退 ~3×**（AOT 172M/JIT 203M，旧 53M/58M 是 builtin map+string 的 C 层数字）——「全部纯 LS」的累计代价，非 bug；f-string codegen（§1/§2）逐行确认未回退。profile 定位新瓶颈＝map 键路径哈希 + Str 堆churn。**两刀**：① **Str.hash → runtime C `__ls_fxhash_bytes`**（逐位等价旧 LS 循环，接线同 `__ls_str_find`：builtins.c+jit.c REG(92)+c.ls extern+str.ls 改调）——杀 250 万次 LS fx_mix 调用、profiler 哈希 25%→1.8%，但墙钟仅 +2%/+6%（**profiler 自时间高估微调用**：每调用插桩，真实未插桩 LLVM 早内联）；仍保留（架构对、长键 O(1)）。② **`codegen_addr_of` 支持 `*T` pointer-index 借用**（**本轮最大收益**）——根因：addr_of 的 AST_INDEX 只认 `TYPE_ARRAY`，`*K` 裸指针下标返回 NULL → 方法接收者落 rvalue-self spill **深克隆 has_drop 槽**；Map 探测 `K existing=self.keys[idx]; existing==k` 每次克隆槽键。修＝加 TYPE_POINTER 分支复用 `codegen_lvalue_ptr` 的 typed GEP + map.ls 改 `self.keys[idx]==k` 就地借用（POD/has_drop 均安全，POD 的 `==` 内建本不克隆）+ `_find` 改 `&K`（内部安全）。**消除 653K per-probe 克隆**。③ **泛型 `&K` 对 POD pointee 退化按值 ABI**（解锁 `get/has?/remove/_find` 改 `&K` 削尽读路径 param 克隆）——根因：`&K` 实例化到 `&int` 时 `type_to_llvm(TYPE_REFERENCE)` 统一发 pointer，但 body 把 `k` 当 int 值用（checker 侧 `&T` 参数有效类型本就是 pointee，已一致）→ `Map(int,int).get` 验证失败。修＝**纯 codegen 两处**（checker 无需改）：`cg_ref_pointee_is_byval`（非 struct/enum/array 即标量）→ ① type_to_llvm 对 read-only `&标量` 返回 pointee 值类型 ② 参数注册按聚合性分流，标量 pointee 解包落值参数路径（调用点 5608 本就对非聚合实参传值，自动匹配）。has_drop K（Str）仍指针借用零克隆、POD K 按值无回退。**累计 AOT 172→103.3M（1.67×）、JIT 203→117.2M（1.74×）**，对 C++ 6.9×→4.1×，克隆 1.46M→0.57M。剩余 565K 克隆是 set 存键路径（语义必需）+ SSO 可攻的 reserve/__drop 半边。ctest 218/218（零 flake）| ✅ |

**当前测试**：ctest 218/218（分支 `feat/std-tensor` → 已合并 main）：std.tensor 堆张量 + std.complex + std.fft 全部完成（编译器：静态 trait 方法 + 泛型 trait impl + ambient builtin 模块解析 + `T.zero()` 类型参派发；根治 `if/while (expr)<op>y {` 老坑）；218=203 + std.tensor 9 测试（phase0/1/2/3/3b/3c/3d/mlp + zero_trait）+ complex/fft 6 测试（complex/tensor_complex/fft/fft_arbitrary/fft_real_dct/fft_nd）。**历史过程见下**：前序分支 `fix/s13-borrow-gaps`：ctest 203/203，plan_std_map §13 挂账**全部清零**——双修 + Block(&!T) 收官；203=201+test_s13_borrow_gaps+test_block_mutref。前序 `feat/get-option`：get→Option(T) 重构完成；201=200+test_vec_get_option。前序 main：std.str 性能 P0/P1/P2 完成 + P5-4 遗留 bug 清账 A/B/C/D-1 完成；200=199 + test_str_perf_methods；199=192 + 新增回归 test_nested_decl_reject/explicit_drop_reject/forward_ref_struct_reject/match_concat_temp/impl_imported_struct/fs/struct_print_str。历史过程记录见下：分支 `feat/string-to-stdlib`：string→Str 下沉 **P5-4 全部完成，builtin string 已不存在**；192=205 减阶段A删除的机制测试。历史过程记录：P7 std 模块迁移 17/20+3 冻结、P4 &string 拆除、P5-0 Str.c_str() 完成；P5-1 样本批量迁移进行中：试点批 8 样本（e894a7b）+ plot 族批 10 样本（69a7432，含 **std/plot.ls 完整 Str 化**）+ **closure 族批 11 样本**（phase_c5/c7、e1、e2_e4、f1~f5、f7_stress、g）+ **第 14 个编译器修复**：闭包 by-move 捕获的 `Option(Str)` 作 match 主体 → `Some(s)=>{return s}` 臂内早返时 AST_RETURN 对 borrowed has_drop struct/enum binder 走 transfer 而非 clone → 返回别名 + env_drop 双释（0xC0000374；builtin string 靠 cap=BORROWED 幸存）；修复＝`borrowed_string` 泛化为 `borrowed_heap`（borrowed has_drop 返回也深拷，[src/codegen.c](src/codegen.c) AST_RETURN）。**否决**更宽的 bind-站点 binder-clone（破坏自递归 JsonValue，已回退）+ **enum 族批 13 样本**（9f764be：enum_string/e1/var_decl/nested_vec/_repro/has_drop_vec/user_vec/vec_payload/method_basic/static/has_drop/borrow/borrow_b）——**自递归 borrow-match 零修复**（enum_borrow_b 的 Jv=JArr(Vec(Jv))，借用 Str binder 经 f-string 内插返回新 owned 不触发第 14 修）；**新友点**：bare match-expr 臂返回字面量（`match self {Red=>"red"}` 作 method 体）不 plumb expected_type=Str → 须改显式 `{ return "..." }`（第三个无桥位）；static `match s {"red"=>...}` string-switch 对 Str 不适用→if/eq? 链；owned 载荷加 `owned(Str)` helper；见 [docs/plan_p5_remove_builtin_string.md](docs/plan_p5_remove_builtin_string.md)）+ **struct/destructor 族批 14 样本**（d4f6be9：struct_field_defaults×3/byval_arg/field_readthrough/return×2/string_copy/string_e2e/string_field×2 + destructor v3/nested/simple）——**零编译器修复**。实证：字段默认/struct字面量字段/字段赋值/嵌套字段赋值的 literal→Str **自动 coerce**（plumb expected_type=field_type，非无桥位）；`Map(string,int)→Map(Str,int)` 键（Str 已 Hash+Eq）。**新坑（待 P5-2/P6）**：`print(整 struct 含 Str 字段)` 渲染成 `Str{data=指针,len,cap}` 非文本（含指针→非确定），逐字段打印规避，翻默认后普遍撞、需 struct auto-print 识别 Str 字段。**跳过**：struct_assign_test（move 后用源，既存 move-error）/complex_destructor(+v2)（函数内嵌套 fn/struct 定义→codegen no-terminator）/nested_destructor_e2e（无括号单行 if parse 错）——均与 string 正交既存损坏；test_mem_m* 族（A 类 string 机制专属，P5-4 删候选）+ **io/fs/map 族批 11 样本**（ceb37ee：io/fs/loop 4＝fs_test/string_loop_test/io_readfile_test/io_step_test + map 族 7＝map_test/map_keys/map_literal_test/map_iter_test/map_index_test/map_compose_test/map_option_payload_test）——**零编译器修复**。**批选逻辑**：聚焦 string 在可桥接位（var_decl/struct 字段/enum payload/call-arg）的样本；string 方法测试族（test/parse/batch3/utils）大量是裸字面量接收者 `"x".method()` 无桥位 → 留 P5-2 翻默认自动迁移，本批不碰。**新实证**：`Map(string,*)→Map(Str,*)` 键 + `m.set("lit",v)`/`get_si(m,"lit")` 字面量参数（方法位 & 自由函数位）均自动 coerce 到 Str；`m[k] == "lit"` 索引结果比较 → `m[k].eq?("lit")`；`e.key.length`/`v.length`→`.len()`；自递归 has_drop enum `JV=Obj(Map(Str,JV))` 的 OPTPAY 回归零 escape-clone。**A 类跳过**（P5-4 删候选）：string_test(LsString 机制 e2e)/string_ord_test(Str 无 Ord impl)/string_append_test(builtin += 优化)/string_memory_e2e/test_mem_m*。**发现** `tests/test_fs.cmake` 未在 CMakeLists 注册（孤儿驱动），fs_test 实为未注册，手动验证 ALL PASS + memcheck 0/0/0。+ **impl Ord for Str + string_ord 迁移**（4784cea）：Str 补 `impl Ord for Str { fn <(...)=compare(...)<0 }`（`> <= >=` 编译器自动派生），P5-2 翻默认前置（`"a"<"b"`/`Vec(Str).sort()`/`min_ord(T:Ord)(Str)` 届时变 Str 比较）；string_ord_test 随之解除 A 类阻塞迁 Str（裸字面量操作数 `"a"<"b"` 落 Str 局部）。+ **map_owndrop/strconv/json 族批 3 样本**（aa12c21：map_owndrop_test/strconv_test/json_file_test）——**零编译器修复**。map_owndrop＝has_drop K/V 所有权（Map(Str,*)/Map(*,Str)/嵌套+struct 字段，f-string 键值经 set/get/remove/rehash 全程 0/0/0）；strconv/json＝FFI/库消费方，全 `Str x = mod.fn(...)` var-decl + `.compare("lit")` 字面量 coerce + json 大数组 `big=big+","`/`big=big+f"{idx}"`（Str+字面量/f-string 拼接经 impl Add）。**A 类跳过新增**：glue_funcs_test(FFI string↔C ABI to_cstr/from_cstr/extern strlen(string)，红线禁 Str 直传 extern)/to_string_test(builtin 全局 to_string/from_* 自身待 P5)/vec_string_test(裸字面量接收者 `"x".upper()` 重灾，留 P5-2)。+ **Option/Result 族批 3 样本**（5bfc522：option_result_test/try_basic_test/force_unwrap_test，均未注册 diff+memcheck 0/0/0）——零编译器修复。`Result(int,string)→Result(int,Str)`/`Option(string)→Option(Str)`/`Err("..")`·`Some("..")` payload 字面量 coerce/force-unwrap owned Str move（`Str su=(os)!`）/`.length`→`.len()`/`"FAIL: "+name`→f-string。**撤回不纳入**：opt_combinator_test（撞真缺口——编译器 lower 的组合子 arg 字面量 `ok_or("missing")`/`unwrap_or("clean")` 不 plumb expected_type=Str → 默认 string 与 Str 载荷不符，同裸字面量接收者留 P5-2）；regex_test（先撤回——**发现预存非确定性 bug**，下条已修）。+ **修 std.regex 非确定 bug + 假绿根治 + regex_test 迁 Str**（cb6f914）：根因＝std/regex.ls 构建返回 Vec(Str)/Map(Str,Str) 时 `string X=st.substr(); push/set(X)` 走 **B-2 call-arg string→Str 桥进入移动型消费者**（Vec.push/Map.set owned-param ABI 移入容器）→ 所有权双重处理 UAF（非确定比较失败）+ find 的 `string m; Str ms=m` IDENT 中间变量克隆源未释放（16字节泄漏）。修＝7 处全改模块内直接 `Str`（var_decl/rvalue 桥，避开进移动消费者；FFI 边界仍 string）。**latent 编译器 bug 记账**：B-2 string→Str call-arg 桥 + 移动消费者不安全（任何 `vec_of_str.push(string_var)` 撞），P5 删 string 后消失。**假绿根治**：test_regex.cmake 原只 grep "ALL PASS"、无 memcheck、不查 FAIL → 加 FAIL 否决断言 + memcheck 0/0/0 断言（教训：审计其它 std 测试驱动同类假绿）。+ **vec/stack 族批 6 样本**（42cb407：vec_batch_b/c/d/e 未注册 + vec_sort_test/stack_test 注册）——零编译器修复。`Vec(string)→Vec(Str)`/`Stack(string)→Stack(Str)`，push/insert/has?/index_of/resize 字面量参数 coerce，`v[i]==lit`/`top==lit`→`.eq?()`。**实测新 impl Ord for Str**：vec_sort/vec_batch_e 的 `Vec(Str).sort()` + sort_by 闭包 `a<b` 走新 Ord 稳定排序 0/0/0。区别于 regex 撞的「string 变量 push 进 Vec(Str)」（B-2 桥进移动消费者 UAF），本批元素全程 Str + 字面量直接 coerce 无中转。+ **vec-functional/rawvec 原语族批 21 样本 + rawvec_* 7 样本改名**（06deade）——零编译器修复。**改名（历史遗留清理）**：std/rawvec.ls 库本体早已被 std.vec 取代删除；7 个内容已是 `import std.vec` 的样本 rawvec_{api,functional_p3,kid_lazy,kid_missing_eq_fail,m2,map_reduce,parity_p1}→vec_*（与注册名 test_vec_* 对齐，6 个 .cmake 驱动同步）；手写原语 gate（m1/move/poc/ptr_index/realloc/sizeof）保留 rawvec_ 名（测 sizeof/realloc/*T 下标/__drop_at 编译器原语，是 std 容器地基，非死代码）。**迁移**：vec_functional_v1~v5（闭包 `s.len()`/`.eq?()`）+ vec_api/m2/kid×2/parity_p1/functional_p3/map_reduce + rawvec_m1/move/poc/ptr_index/sizeof + inferred_init/implicit_empty_init/fn_default_params/trait_basic（未注册 diff 基线）。**新实证（本批新 territory）**：`*Str` 裸缓冲容器全套内存安全（`std.c.realloc`+`sizeof(Str)`+`__drop_at(slot)`+`p[i]` 深拷经 Str.__clone+`__move`，rawvec_m1 三层嵌套 0/0/0）；方法级泛型 `map(Str)`/`reduce(Str)` 闭包体 f-string/字段读（`|x| f"v={x}"`/`|p| p.name`）plumb expected 正常；**默认参数值字面量位** `Str sep = "-"` 自动 coerce；trait 方法签名 `-> Str` + 受约束泛型返回 Str；`Map(Str,int)` 无 init（M-DEF 合成 `{}`）。坑：`f"" + s`（f-string 作 + 接收者位）不支持→改 `f"{s}"`。**defer P5-2 新增**：vec_owndrop/vec_global_drop/deep_copy_all/vec_literal/hash_test（`"x".upper()`/`"x".hash()` 裸字面量接收者作测试主体）。+ **P5-2 预研：实验开关 `LS_STR_DEFAULT=1` 翻字面量默认**（[docs/plan_p5_remove_builtin_string.md](docs/plan_p5_remove_builtin_string.md) §5.1）：4 文件 env 守卫（ast.c 根程序注入 `import std.str`、main.c/jit.c 四入口、checker AST_STRING_LIT/AST_FORMAT_STRING 默认翻 Str 仅 `expected==string` 保 builtin），默认 OFF 零行为变化。**ON 态实测：205→50 失败全部编译期 type error，零运行期失败**（零坏值/零 memcheck/零段错），翻转破坏面 100% 响亮。错误谱 270 条：150 compare + 40 init + 34 field（机械迁移）+ 19 builtin 方法 arg（不补 checker，样本迁完自然消失）+ 7 string-switch。**四个已记账缺口自动消解**：裸字面量接收者 `"x,y".split(",")`/组合子参数 `unwrap_or("fb")`·`ok_or("e")`/bare match 臂字面量/字面量 `+` 拼接（impl Add）。施工顺序定稿（§5.1）：定向收尾批（ON 错误清单驱动，双态验证）→ 12 机制测试改写 → A 类处置 → 正式翻转（prelude 注入+REPL+struct auto-print 识别 Str，保 `LS_STR_DEFAULT=0` 逃生舱）。+ **P5-2 Step 1：定向收尾批①~④ 共 29 样本**（4 提交）——ON 态失败 51→23（**OFF 态全程 205/205 零回归**，每批双态验证）。**批①** 9 样本（比较→`.eq?`/类型→`Str`/补 `import std.str`）：i64_literal/stdlib_path/modvar_access/hasdrop/owned_return/stack_qual/fstring_spec/shortcircuit_temp/ring。**批②** 11 样本（struct 字段/enum 载荷/泛型实参/`Vec(Str)` 元素，含 modstructlit 的 opt.ls 模块迁移）：generics_g1/g2/match_result_own/hash/bf044/bf046/mod_struct_literal/vec_owndrop/iter_protocol/global_vec_lit/move_elision。**批③** 6 样本：match_or_pattern（string-switch→if/eq? 链）/vec_get_unsafe/vec_oob/vec_global_drop/string_parse/bf045_string_param + **Str.to_int/to_i64 补 `0x` 十六进制 parity**（注释明言"matches the old builtin"，string_parse 撞缺口；**坑：`Str.to_bool` 故意严格只认 true/false**——曾补 yes/no/1/0 parity 却撞 str_methods3 断言「yes→Err」，已回退）。**批④** 5 样本：opt_combinator（组合子 `Result(_,Str)`；**`try o.ok_or("e")` / 链式 `.unwrap_or("fb").eq?()` 位不 plumb expected→Str 载荷与 string 字面量不符，须 Str 局部强制**，var_decl 位则自动 plumb）/modtype_memcheck（跨模块 mod_a/b 迁移）/html_write/md_parse（`.contains`→`.contains?`）。**机械规则三类无桥位**（须 Str 局部/eq? 规避）：①裸字面量接收者（`"42".to_int()`→`Str s="42"; s.to_int()`，string_parse/string_utils）②多参泛型字面量实参（`make_pair(int,Str)(99,"w")`→局部 w）③组合子 arg 在 try/链式位。**判 A 类不迁**（留 Step 2/3/P5-4）：impl_string（测 `impl string` builtin 扩展特性；`impl Str` 对导入结构体符号不带模块前缀→"Symbols not found"，需编译器修）+ string_utils（builtin std.string `to_bool` 宽松语义 yes/no/1/0，Str 故意严格不复刻）+ glue_funcs（FFI 红线禁 Str 传 extern，但仅裸 `"lit".to_cstr()` 受 flip 影响→pin string 局部屏蔽，已迁过）。**剩 23 ON 失败＝12 str_* 桥机制自测（Step 2 改写）+ 8 memcheck A 类 + impl_string + string_utils + (m5_aot 偶发)**，全 builtin-string 机制专属。+ **P5-2 Step 2：12 个 str_*/桥机制测试改写双态绿**（3 提交）——ON 态失败 **23→9**（OFF 态 205/205 零回归）。这 12 个**与 B 类样本本质不同**：故意保留 `string` 类型值验证 string↔Str 双向桥，**不能无脑全迁 Str**（会删桥覆盖）。**批①** str_p0/p1/p2/p3：`X.to_string() == "lit"`（string LHS，被测的 Str→string 桥）→ 加 `fn seq(string,string)->bool { return a==b }` 助手，字面量经 string 参数位 coerce 回 builtin（保 to_string + string 相等覆盖，双态绿）。**批②** str_methods/methods2/lit_borrow/fstring_interp：`.to_string() == "lit"` → `.eq?("lit")`（无需捕获 LHS 更稳健，LHS 是 Str rvalue；坑：含方法实参空格的 LHS 用 seq-wrap 会捕获错位，故用 .eq?；to_string 覆盖由 str_p* 保留）；str_fstring_interp 的 `string bs = f"..."` 测 builtin f-string，钉 `string bs_exp` 局部保 builtin 比较。**批③** str_bridge_arg/ret/rev/s2s（最微妙）：Str-LHS `.to_string()==`→`.eq?`；**string-LHS 桥源/产物比较**（`sv=="hello"` 测桥后源存活、`a=="hello"` 测反向桥产物是 builtin string）→ seq(string,string) 保 string==string；**builtin string `ov.append("amic")`/`f + "!"` 的字面量实参**钉 string 局部（flip 下字面量变 Str → `string.append(Str)` 报错）。**红线守住**：各测试原本测的桥方向（B-1/B-2/B-2b/B-3）的桥调用本身不改，只改断言侧。**零编译器/零库修复**，全 memcheck 0/0/0。**剩 9 ON 失败＝7 memcheck A 类（mem_m3/m4/m4_5×2/overhaul×2/memcheck_edge）+ impl_string + string_utils**，全 builtin-string 机制专属，留 Step 3（A 类处置：改 Str 版或随 P5-4 删）。+ **P5-2 Step 3：A 类处置——ON 态实质归零**（2 提交）——ON 失败 **9→0 真失败**（残 2 个 memcheck_aot/mem_overhaul_aot 是已知 AOT Defender flake，`--repeat until-pass:2` 即过；OFF 态 205/205）。**两路处置**：① **5 个 memcheck 测试迁 Str**（mem_m3/m4/m4_5/overhaul/memcheck_edge）——经审计它们是**通用 has_drop 内存检查器测试**（用 string 作 owned 载荷测 leak/double-free），非 builtin-string 内部机制专属 → `string→Str` 语义等价迁移（脚本批量类型位替换 + `.length`→`.len()`；m3 的 `enum Value{Str(string)}` 变体名 `Str`→`Text` 避与类型 Str 同名，仿 json 先例）；builtin string 借用 ABI（cap=-2）覆盖仍由 str_bridge_* 四测试保留，**净覆盖不丢**；双态+memcheck 0/0/0。② **impl_string + string_utils 钉 escape-hatch**（`set_tests_properties(... ENVIRONMENT "LS_STR_DEFAULT=0")`）——它们测 frozen builtin-string 特性（`impl string` 扩展 / std.string 宽松 `to_bool` yes/no/1/0），Str 无等价（`impl Str` 对导入结构体符号缺模块前缀 / Str.to_bool 故意严格），故**保留 builtin-string 模式**直到 P5-4 随 builtin string 删除。ctest ENVIRONMENT 属性设入子进程环境覆盖继承的全局 flip，使其在默认翻转后仍测 builtin（实测全局 `LS_STR_DEFAULT=1` 下两测试通过）。**Step 1+2+3 累计：ON 态 51→0 真失败，OFF 态全程 205/205 零回归**；P5-2 翻转前样本/测试侧已全部就绪，剩 Step 4 正式翻转（默认 ON + prelude 注入 + REPL 接线 + struct auto-print 识别 Str）。+ **P5-2 Step 4：正式翻转——字面量/f-string 默认 Str**（1 提交）：实验开关转正为默认行为，**编译器三处改动**——① `checker.c str_default_flip_enabled()` 默认 ON（`LS_STR_DEFAULT=0` 为逃生舱，反转判据 `v[0]=='0'?0:1`）；② `ast.c ast_inject_std_str_import()`（prelude）默认注入 `import std.str` 到根程序（`=0` 跳过；模块不注入避 std.str→std.c 环不变）；③ `jit.c` REPL 路径接线 prelude 注入（每 snippet 重放，与既有 import 重放一致）。**验证**：默认无 env 跑 ctest **205/205**（残 memcheck_aot 是已知 AOT Defender flake，`--repeat` 即过）——与 Step 1-3 全程 `LS_STR_DEFAULT=1` 全局态等价（那些 ON 态跑就是预演翻转）；逃生舱 `=0` 恢复 builtin string 默认（impl_string/string_utils 经 ENVIRONMENT=0 仍测 builtin）；REPL `Str s="hi"; print(s.upper())`→HI 接线生效。**缓到 P6**：struct auto-print 识别 Str 字段（`print(整 struct 含 Str 字段)` 仍渲染 `Str{data=指针,...}`——但 ctest 全绿证明测试套件不依赖，逐字段打印规避；用户面改进留 P6）。**翻转点已过，此后裸字面量 `"x".method()`/`"a"==b`/`+` 操作数/组合子 arg 等无桥位全部天然 Str**，四个历史记账缺口自动消解。剩 **P5-3/P5-4**：解冻 c/os（边界 c_str）+ 删 string.ls/hash string impl（P5-3），原子拆除 builtin string（scanner→checker→codegen→types，P5-4）。+ **P5-3：FFI 边界解冻 c.ls/os.ls**（3 提交）——**c.ls/os.ls 零残留 `string`**（grep 确认）。**关键洞察**：fs/proc/io/env/time 等调用方**已持 Str**，原 `raw_X(string)`/`c.X(string)` 是走 B-3 反向桥（Str→string+NUL）→ 把 extern 签名 `string`→`*u8`、wrapper 收 `Str` 传 `.c_str()`（P5-0 c_str 保 NUL，红线合规）后，**调用方多数零改动**（os.ls 全部 raw_* 集中改，fs/env/time/io 调用方不动）。**os.ls**（1 提交）：~25 extern `string`→`*u8` + raw_* 收 Str 传 c_str；导入 std.str 无环（os→str→{c,vec,hash}）。**c.ls 2a**（1 提交）：fopen/system/strlen→*u8；io.ls 5 个 c.fopen 删 `string sp` 桥改 `path.c_str()+mode.c_str()`（_mode_str→Str）、proc.ls `c.system(cmd.c_str())`；**顺带修预存泄漏**（test_proc 不跑 memcheck 故 Step4 baseline 即漏未抓）：proc.exec() 的 `string raw_out=__string_take_buffer(...)`+`Str out=raw_out` B-3 桥克隆 builtin string 中转每次泄漏 16+19B → 改 io._own_buf 式零拷贝 `Str{data:buf,len,cap}` 直接拥有 c.malloc buffer，proc_test memcheck 0/0/0。**c.ls 2b regex**（1 提交）：__ls_regex_compile/exec→*u8；regex.ls 全 13 函数删 `string st=text` 桥，`text.c_str()`/`.len()`/`.substr`→Str，循环 hoist `*u8 tptr`（c_str 摊还 O(1)），full_match/replace 用 Str `+` 拼接，`_cap_str(&Str)` 借用避克隆。regex_test ALL PASS+0/0/0。**REPL 注入撤销**（os.ls 同提交）：Step4 引入的 per-snippet ast_inject_std_str_import 与 snippet 重放+L-010 交互致 test_repl_import flaky（1/3），撤销后 4/4 稳定，REPL 保 builtin 默认至 P5-4。每提交 ctest 205/205。**P5-3 剩余（耦合 P5-4）**：string.ls（`impl string` 本依赖 builtin string，与其测试 impl_string/string_utils 同步删）+ hash.ls 的 `impl Hash/Eq for string`（仍被 opt_combinator/map_compose/map_iter 等用 `Map(string,*)` 的样本消费，需先迁 Map(Str,*)）——均待 P5-4 原子拆除时一并处置。前序 `feat/match-result-ownership`：L-013 match 结果所有权整改；`feat/opt-combinators-c1`：Option/Result 组合子 C1+C2a；`feat/map-index-protocol`：Map 索引协议）
...
- ~~`vec.get` / `map.get` 改返回 `Option(T)`（breaking change）~~ ✅ 已完成（2026-06-12）：`Map.get` 自 M-0 即 Option；`Vec.get` 经 get→Option 重构落地（[docs/plan_container_access_safety.md](docs/plan_container_access_safety.md) §5.6）
