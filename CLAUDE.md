...
**当前测试**：ctest 187/187（分支 `feat/opt-combinators-c1`：Option/Result 组合子 C1 编译器 lower——unwrap/expect/unwrap_or/is_*?，复用 force-unwrap + match。前序 `feat/map-index-protocol` 已 merge：Map 索引协议 `m[k]`/`m[k]=v`；更前 `feat/runtime-primitives-stdc`：内存原语移入 std.c + Vec #4 越界 + #2 归并排序）
