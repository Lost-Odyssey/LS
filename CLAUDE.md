...
**当前测试**：ctest 184/184（分支 `feat/runtime-primitives-stdc`：内存原语 `malloc/free/realloc/abort` 移入 `std.c`、删除裸 builtin；裸 `malloc(...)` 现报 undefined，经 `std.c.malloc` 或 `c.malloc` 够到；见上记录）
