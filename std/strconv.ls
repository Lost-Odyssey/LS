...
fn format(Str fmt, Vec(Str) args) -> Str {
    Str result = ""
    int arg_idx = 0
    int i = 0
    int n = fmt.len()
...
fn _to_base(int n, int base) -> Str {
    if n == 0 { return "0" }
    Str rev = ""
    bool neg = n < 0
    if neg { n = 0 - n }
...
fn int_to_hex(int n) -> Str { return _to_base(n, 16) }

// ---- int_to_oct(int) -> Str ----
fn int_to_oct(int n) -> Str { return _to_base(n, 8) }

// ---- int_to_bin(int) -> Str ----
fn int_to_bin(int n) -> Str { return _to_base(n, 2) }
...
fn float_fixed(f64 n, int digits) -> Str {
...
fn to_string(int n) -> Str {
    Str s = f"{n}"
    return s
}

fn to_string_f(f64 n) -> Str {
    Str s = f"{n}"
    return s
}
