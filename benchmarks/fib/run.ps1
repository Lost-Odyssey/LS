...
Write-Host "=== fib benchmark (n=$N, iters=$Iters) ===" -ForegroundColor Cyan
...
function Run-Native {
    param([string]$Desc, [string]$OutFile)
    Write-Host "  $Desc ..." -ForegroundColor Yellow
    # Run from $FibDir for all language runners so relative paths work
    Push-Location -LiteralPath $FibDir
    try {
        # Capture stdout cleanly using temporary file
        $tmpOut = Join-Path $FibDir "_stdout.tmp"
        Remove-Item -LiteralPath $tmpOut -Force -ErrorAction SilentlyContinue
        Invoke-Expression -Command $Desc | Out-File -FilePath $tmpOut -Encoding utf8
        $ec = $LASTEXITCODE
        if (Test-Path $tmpOut) {
            Get-Content -LiteralPath $tmpOut -Raw | Out-File -FilePath $OutFile -Encoding utf8
            Remove-Item -LiteralPath $tmpOut -Force -ErrorAction SilentlyContinue
        }
        if ($ec -ne 0) { Write-Host "    FAILED (exit=$ec)" -ForegroundColor Red; return $false }
        return $true
    } finally { Pop-Location }
}

# Since Invoke-Expression is tricky with native EXEs, use direct & calls:

# ── 1. LS JIT ──
$f = Join-Path $ResultsDir "fib_ls_jit.txt"
Write-Host "  [1/5] LS (JIT) ..." -ForegroundColor Yellow
Push-Location -LiteralPath $FibDir
$ls_jit_out = & $LS run fib.ls $N
...
$lsExe = Join-Path $FibDir "fib_ls_aot.exe"
$ls_aot_ok = $true
& $LS compile $FibDir\fib.ls -o $lsExe | Out-Null
if ($LASTEXITCODE -eq 0) {
    $f = Join-Path $ResultsDir "fib_ls_aot.txt"
    Push-Location -LiteralPath $FibDir
    $ls_aot_out = & .\fib_ls_aot.exe $N
...
$cExe = Join-Path $FibDir "fib_c.exe"
$vcvars = "C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Auxiliary\Build\vcvars64.bat"
cmd /c "`"$vcvars`" > nul 2>&1 && cl /O2 /MT /nologo `"$FibDir\fib.c`" /Fe:`"$cExe`" /Fo:`"$FibDir\fib_c.obj`" > nul 2>&1"
$c_ok = $LASTEXITCODE -eq 0
if ($c_ok) {
    $f = Join-Path $ResultsDir "fib_c.txt"
    $c_out = & $cExe $N
    $c_ec = $LASTEXITCODE
...
$f = Join-Path $ResultsDir "fib_py.txt"
Write-Host "  [4/5] Python (1 iter) ..." -ForegroundColor Yellow
$py_out = python $FibDir\fib.py $N 1
$py_ec = $LASTEXITCODE
...
$f = Join-Path $ResultsDir "fib_rb.txt"
Write-Host "  [5/5] Ruby (1 iter) ..." -ForegroundColor Yellow
$rb_out = ruby $FibDir\fib.rb $N 1
$rb_ec = $LASTEXITCODE
...
$results = @{
    "LS (JIT)" = if ($ls_jit_ok) { Extract-Result (Join-Path $ResultsDir "fib_ls_jit.txt") } else { @{r="N/A";m="N/A"} }
    "LS (AOT)" = if ($ls_aot_ok) { Extract-Result (Join-Path $ResultsDir "fib_ls_aot.txt") } else { @{r="N/A";m="N/A"} }
    "C (MSVC)" = if ($c_ok)      { Extract-Result (Join-Path $ResultsDir "fib_c.txt") }       else { @{r="N/A";m="N/A"} }
    "Python"   = if ($py_ok)     { Multiply-Mean10 (Extract-Result (Join-Path $ResultsDir "fib_py.txt")) } else { @{r="N/A";m="N/A"} }
    "Ruby"     = if ($rb_ok)     { Multiply-Mean10 (Extract-Result (Join-Path $ResultsDir "fib_rb.txt")) } else { @{r="N/A";m="N/A"} }
}

$lines = @(
    "fib benchmark results",
    "======================",
...
$summary | Out-File -FilePath (Join-Path $ResultsDir "fib_summary.txt") -Encoding utf8
