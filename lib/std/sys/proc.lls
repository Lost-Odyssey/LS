...
// All OS differences handled by std.sys.os; C bindings via std.sys.c.
...
def run(Str cmd) -> int {
    /* c.system is a DIRECT extern def taking *u8 (NUL-terminated char*).
...
def exec(Str cmd) -> Result(Str, Str) {
...
def _copy_buf(object p, i64 n) -> Str {
    Str s = ""
    if p == nil { return s }
    int len = n as int
    s.reserve(len)
...
def exec_full(Str cmd) -> Result(ExecResult, Str) {
