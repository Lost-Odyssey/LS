...
impl(T) Complex(T) {
    ...
    static fn zero() -> Complex(T) { return {re: 0 as T, im: 0 as T} }
    static fn one()  -> Complex(T) { return {re: 1 as T, im: 0 as T} }
    // imaginary unit i
    static fn i_unit() -> Complex(T) { return {re: 0 as T, im: 1 as T} }

    fn conj(&self) -> Complex(T) { return {re: self.re, im: 0 as T - self.im} }
    // squared magnitude re^2 + im^2 (no sqrt; valid for any numeric T)
    fn norm(&self) -> T { return self.re * self.re + self.im * self.im }
    // scalar multiply
    fn scale(&self, T s) -> Complex(T) { return {re: self.re * s, im: self.im * s} }

    // --- float-only (math.*) — instantiate only for float T ---
    fn abs(&self) -> f64 { return math.sqrt(self.norm()) }
    fn arg(&self) -> f64 { return math.atan2(self.im, self.re) }
    // e^z = e^re * (cos im + i sin im)
    fn exp(&self) -> Complex(T) {
        T er = math.exp(self.re)
        return {re: er * math.cos(self.im), im: er * math.sin(self.im)}
    }
    static fn from_polar(T r, T theta) -> Complex(T) {
...
impl(T) Add for Complex(T) {
    fn +(&self, &Complex(T) rhs) -> Complex(T) {
...
// rectangular: c(re, im)
fn c(T)(T re, T im) -> Complex(T) { return {re: re, im: im} }
// real axis: real(x) = x + 0i
fn real(T)(T re) -> Complex(T) { return {re: re, im: 0 as T} }
// polar: from_polar(r, theta) = r*(cos theta + i sin theta)  (float-only)
fn from_polar(T)(T r, T theta) -> Complex(T) {
    return {re: r * math.cos(theta), im: r * math.sin(theta)}
}
