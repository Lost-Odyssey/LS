...
//
// Construction (the `{}` + init idiom — generic free-def constructors are not yet
...
// extern def / BLAS. The pointer is valid only while the tensor is alive (no
// lifetime system — same red line as Vec.as_ptr / map by-ref capture).
//
// NOTE: runtime primitives are reached via the canonical std.sys.c.malloc/free/abort
// path (literal-matched, no `import std.sys.c as c` alias — that alias is invisible
// when a generic method is instantiated in the consumer module, same as std.core.vec).
...
// expression `zeros(f64)(sh)`. They are GENERIC FREE FUNCTIONS — `import std.sci.tensor`
// registers them into the caller's def-template registry, so they are called by
...
// calls the cross-module-safe _setup method + std.sys.c.malloc), so it instantiates
// cleanly in any importer.

// zeros(T)(sh): a sh-shaped tensor of 0.
def zeros(T)(Vec(int) shape) -> Tensor(T) {
    Tensor(T) out = {}
    int n = out._setup(shape);
    *T p = std.sys.c.malloc(n * sizeof(T)) as *T
    int i = 0
...
def full(T)(Vec(int) shape, T fill) -> Tensor(T) {
    Tensor(T) out = {}
    int n = out._setup(shape);
    *T p = std.sys.c.malloc(n * sizeof(T)) as *T
    int i = 0
...
// resolution gap in generic-free-def instantiation, at the cost of one copy.
def from_vec(T)(Vec(int) shape, Vec(T) src) -> Tensor(T) {
    Tensor(T) out = {}
    int n = out._setup(shape);
    if src.len() != n {
...
methods Tensor(T) {
    ...
    def _setup(&!self, &Vec(int) shape) -> int {
        int r = shape.len()
        int n = 1
        int k = 0
    ...
    def init(&!self, &Vec(int) shape, T fill) {
    ...
    def init_zeros(&!self, &Vec(int) shape) { self.init(shape, T.zero()) }

    // Allocate from a flat row-major Vec(T); src.len() must equal prod(shape).
    def init_from(&!self, &Vec(int) shape, &Vec(T) src) {
    ...
    // ---- queries ----
    def rank(&self) -> int { return self.shape.len() }
    def size(&self) -> int { return self.size }
    def dim(&self, int k) -> int { return self.shape.get!(k) }
    def shape(&self) -> Vec(int) { return self.shape.copy() }
    def strides(&self) -> Vec(int) { return self.strides.copy() }
    def as_ptr(&self) -> *T { return self.data }
    def as_mut_ptr(&!self) -> *T { return self.data }
    ...
    def set4(&!self, int i, int j, int k, int l, T v) {
        self._ckdim(0, i)
        self._ckdim(1, j)
        self._ckdim(2, k)
        self._ckdim(3, l)
    ...
    def __index2(&self, int i, int j) -> T { return self.at2(i, j) }
    def __index3(&self, int i, int j, int k) -> T { return self.at3(i, j, k) }
    def __index4(&self, int i, int j, int k, int l) -> T { return self.at4(i, j, k, l) }
    def __index_set2(&!self, int i, int j, T v) { self.set2(i, j, v) }
    def __index_set3(&!self, int i, int j, int k, T v) { self.set3(i, j, k, v) }
    def __index_set4(&!self, int i, int j, int k, int l, T v) { self.set4(i, j, k, l, v) }
    ...
    // Vec's where T: Order). Elementwise binary ops follow NumPy broadcasting.
    ...
    def _bcast_plan(&self, &Tensor(T) o) -> BCast {
    ...
    def _bcast(&self, &Tensor(T) o, int opcode) -> Tensor(T) {
        BCast bc = self._bcast_plan(o)
        Tensor(T) out = {}
        int total = out._setup(bc.shape);
        *T p = std.sys.c.malloc(total * sizeof(T)) as *T
    ...
    // across rows.
    def add(&self, &Tensor(T) o) -> Tensor(T) { return self._bcast(o, 0) }
    def sub(&self, &Tensor(T) o) -> Tensor(T) { return self._bcast(o, 1) }
    def mul(&self, &Tensor(T) o) -> Tensor(T) { return self._bcast(o, 2) }
    def div(&self, &Tensor(T) o) -> Tensor(T) { return self._bcast(o, 3) }

    // Scalar broadcast: every element OP s → new Tensor of the same shape.
    def add_scalar(&self, T s) -> Tensor(T) {
        Tensor(T) out = {}
    ...
    def scale(&self, T s) -> Tensor(T) {            // multiply every element by s
        Tensor(T) out = {}
        int n = out._setup(self.shape);
        *T p = std.sys.c.malloc(n * sizeof(T)) as *T
        int i = 0
    ...
    def mean(&self) -> T { return self.sum() / (self.size as T) }
    def max(&self) -> T {                            // assumes size >= 1
        T m = self.data[0]
        int i = 1
        while i < self.size {
            T v = self.data[i]
    ...
    def argmax(&self) -> int {                       // flat index of the max element
        int idx = 0
        T m = self.data[0]
        int i = 1
        while i < self.size {
    ...
    def sum_axis(&self, int axis) -> Tensor(T) {
    ...
    def mean_axis(&self, int axis) -> Tensor(T) {
    ...
    def max_axis(&self, int axis) -> Tensor(T) {
        int r = self.shape.len()
        if axis < 0 || axis >= r {
    ...
    // when it is not shadowed by a local (mirroring the ambient std.sys.c.* path).
    ...
    def softmax_rows(&self) -> Tensor(T) {
        if self.shape.len() != 2 {
    ...
    def matmul(&self, &Tensor(T) o) -> Tensor(T) {
    ...
    def row(&self, int i) -> Tensor(T) {
        if self.shape.len() != 2 { @print(f"Tensor.row requires 2-D"); std.sys.c.abort() }
        self._ckdim(0, i)
        int n = self.shape.get!(1)
        int s0 = self.strides.get!(0)
        int s1 = self.strides.get!(1)
    ...
    def col(&self, int j) -> Tensor(T) {
        if self.shape.len() != 2 { @print(f"Tensor.col requires 2-D"); std.sys.c.abort() }
        self._ckdim(1, j)
        int m = self.shape.get!(0)
        int s0 = self.strides.get!(0)
        int s1 = self.strides.get!(1)
    ...
    def slice(&self, int axis, int lo, int hi) -> Tensor(T) {
        int r = self.shape.len()
        if axis < 0 || axis >= r { @print(f"Tensor.slice bad axis {axis}"); std.sys.c.abort() }
    ...
    def argmax_rows(&self) -> Vec(int) {
        if self.shape.len() != 2 { @print(f"Tensor.argmax_rows requires 2-D"); std.sys.c.abort() }
        int m = self.shape.get!(0)
        int n = self.shape.get!(1)
        int s0 = self.strides.get!(0)
        int s1 = self.strides.get!(1)
    ...
    def sqrt(&self) -> Tensor(T) {          // float only (math.sqrt)
        Tensor(T) out = {}
        int n = out._setup(self.shape);
        *T p = std.sys.c.malloc(n * sizeof(T)) as *T
        int i = 0
    ...
    def log(&self) -> Tensor(T) {           // natural log, float only (math.log)
        Tensor(T) out = {}
        int n = out._setup(self.shape);
        *T p = std.sys.c.malloc(n * sizeof(T)) as *T
        int i = 0
    ...
    def clamp(&self, T lo, T hi) -> Tensor(T) {
        Tensor(T) out = {}
        int n = out._setup(self.shape);
        *T p = std.sys.c.malloc(n * sizeof(T)) as *T
        int i = 0
    ...
    def reshape(&!self, &Vec(int) newshape) {
        int r = newshape.len()
        int n = 1
        int k = 0
