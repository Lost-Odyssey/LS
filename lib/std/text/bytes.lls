...
//
// Import:  import std.text.bytes as b
//
...
def load_u8(*u8 buf, int off)    -> u8  { return c.__ls_load_u8(buf, off) as u8 }
def load_be_u16(*u8 buf, int off) -> u16 { return c.__ls_load_be_u16(buf, off) as u16 }
def load_be_u32(*u8 buf, int off) -> u32 { return c.__ls_load_be_u32(buf, off) as u32 }
def load_be_u64(*u8 buf, int off) -> u64 { return c.__ls_load_be_u64(buf, off) }
def load_le_u16(*u8 buf, int off) -> u16 { return c.__ls_load_le_u16(buf, off) as u16 }
def load_le_u32(*u8 buf, int off) -> u32 { return c.__ls_load_le_u32(buf, off) as u32 }
def load_le_u64(*u8 buf, int off) -> u64 { return c.__ls_load_le_u64(buf, off) }
...
def of_bytes(Str buf) -> Reader { return Reader{ buf: buf, pos: 0 } }

// BORROW: wrap an existing *u8 + len as a non-owning Str (cap==0 → drop won't free).
def borrow(*u8 p, int len) -> Reader { return Reader{ buf: Str{ data: p, len: len, cap: 0 }, pos: 0 } }

methods Reader {
    def pos(&self) -> int       { return self.pos }
    def len(&self) -> int       { return self.buf.len() }
    def remaining(&self) -> int { return self.buf.len() - self.pos }
    def eof?(&self) -> bool     { return self.pos >= self.buf.len() }
    def has?(&self, int n) -> bool { return self.pos + n <= self.buf.len() }

    def skip(&!self, int n)   { self.pos = self.pos + n }   // advance over n bytes
    def seek(&!self, int off) { self.pos = off }            // absolute reposition
