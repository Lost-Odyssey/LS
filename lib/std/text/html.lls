...
def text(Str s) -> HtmlNode { return Text(s) }
def comment(Str s) -> HtmlNode { return Comment(s) }
def raw(Str s) -> HtmlNode { return RawText(s) }

// Single attribute.
def attr(Str k, Str v) -> Attr { return Attr { key: k, val: v } }

// Build an attribute list from [[key, value], ...] pairs (LS has no named args).
def attrs(Vec(Vec(Str)) pairs) -> Vec(Attr) {
    Vec(Attr) out = {}
    int i = 0
    while i < pairs.len {
...
def elem(Str tag, Vec(HtmlNode) children) -> HtmlNode {
    Vec(Attr) a = {}
    return Element(tag, a, children)
}

// Element with attributes. Both `a` and `children` are moved into the node.
def elem_attr(Str tag, Vec(Attr) a, Vec(HtmlNode) children) -> HtmlNode {
    return Element(tag, a, children)
}

// ---- Convenience constructors ----

def div(Vec(HtmlNode) children) -> HtmlNode { return elem("div", children) }
def span(Vec(HtmlNode) children) -> HtmlNode { return elem("span", children) }
def ul(Vec(HtmlNode) children) -> HtmlNode { return elem("ul", children) }
def ol(Vec(HtmlNode) children) -> HtmlNode { return elem("ol", children) }
def li(Vec(HtmlNode) children) -> HtmlNode { return elem("li", children) }

// <hN>text</hN>
def h(int level, Str s) -> HtmlNode {
    int lv = level
    if lv < 1 { lv = 1 }
    if lv > 6 { lv = 6 }
    Str tag = f"h{lv}"
...
// <a href="url">text</a>
def a(Str anchor_text, Str url) -> HtmlNode {
    Vec(Attr) at = {}
...
def img(Str src, Str alt) -> HtmlNode {
    Vec(Attr) at = {}
...
def document(Vec(HtmlNode) roots) -> HtmlDoc { return HtmlDoc { roots: roots } }
def fragment(Vec(HtmlNode) nodes) -> HtmlDoc { return HtmlDoc { roots: nodes } }

// ---- Escaping ----

// Escape text content: & < >  (read-only by-value param, like md.ls _pad_right)
def escape(Str s) -> Str {
    Str r = ""
    int i = 0
    int n = s.len()
    while i < n {
...
def _render_node(HtmlNode n) -> Str {
    match n {
...
def _render_node_pretty(HtmlNode n, int depth, int step) -> Str {
...
def render_pretty(&HtmlDoc d, int step) -> Str {
    Str out = ""
    int i = 0
    while i < d.roots.len {
...
def fmt_tag(Str tag, Vec(Vec(Str)) attr_pairs, Str inner) -> Str {
    Str r = "<"
    r.push_str(tag)
    int i = 0
...
def _hp_matches(&!HtmlParser p, Str s) -> bool {
    int n = s.len()
...
def _hp_matches_ci(&!HtmlParser p, Str s) -> bool {
    int n = s.len()
    if p.pos + n > p.len { return false }
    int i = 0
    while i < n {
...
def _hp_lower(Str s) -> Str {
    return s.lower()
}

def _hp_is_name_end(int c) -> bool {
    return c < 0 || c == ' ' || c == '\t' || c == '\n' || c == '\r'
        || c == '>' || c == '/' || c == '='
}

def _hp_scan_tag_name(&!HtmlParser p) -> Str {
    int start = p.pos
...
def _decode_numeric_entity(Str ent) -> int {
...
def _decode_entities(Str s) -> Str {
    Str r = ""
    int i = 0
    int n = s.len()
    while i < n {
...
def _hp_is_attr_name_end(int c) -> bool {
    return c < 0 || c == ' ' || c == '\t' || c == '\n' || c == '\r'
        || c == '=' || c == '>' || c == '/'
}

def _hp_scan_attr_name(&!HtmlParser p) -> Str {
    int start = p.pos
...
def _hp_scan_attr_value(&!HtmlParser p) -> Str {
    int q = _hp_peek(&!p)
    if q == '"' || q == '\'' {
...
def _parse_attrs(&!HtmlParser p) -> Vec(Attr) {
    Vec(Attr) out = {}
    while p.pos < p.len {
        _hp_skip_ws(&!p)
        int c = _hp_peek(&!p)
...
def _parse_comment(&!HtmlParser p) -> HtmlNode {
...
def _parse_doctype(&!HtmlParser p) -> HtmlNode {
...
def _parse_text(&!HtmlParser p) -> HtmlNode {
    int start = p.pos
    while p.pos < p.len && p.input.byte_at(p.pos) != '<' {
        p.pos = p.pos + 1
    }
...
def _hp_scan_raw_until_close(&!HtmlParser p, Str tag) -> Str {
    Str close = "</"
...
def _parse_element(&!HtmlParser p) -> HtmlNode {
    p.pos = p.pos + 1                    // consume '<'
...
def _parse_nodes(&!HtmlParser p, Str close_tag) -> Vec(HtmlNode) {
    Vec(HtmlNode) nodes = {}
    while p.pos < p.len {
        int c = p.input.byte_at(p.pos)
        if c == '<' {
...
def get_attr(HtmlNode n, Str key) -> Str {
    match n {
        Element(tag, attrs, children) => {
            int i = 0
            while i < attrs.len {
                &Attr at = attrs.get_ref(i)
...
def _collect_links(HtmlNode n) -> Vec(Str) {
    Vec(Str) acc = {}
    match n {
        Element(tag, attrs, children) => {
            if tag == "a" {
...
def extract_links(&HtmlDoc d) -> Vec(Str) {
    Vec(Str) acc = {}
    int i = 0
    while i < d.roots.len {
...
def _collect_by_tag(HtmlNode n, Str tag) -> Vec(HtmlNode) {
    Vec(HtmlNode) acc = {}
    match n {
...
def find_by_tag(&HtmlDoc d, Str tag) -> Vec(HtmlNode) {
    Vec(HtmlNode) acc = {}
    int i = 0
    while i < d.roots.len {
