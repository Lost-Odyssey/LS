...
def _inline_text(Str s) -> Vec(MdInline) {
    Vec(MdInline) v = {}
    v.push(Text(s))
    return v
}

def _repeat_char(int ch, int n) -> Str {
    Str s = ""
    int i = 0
    while i < n {
        s.push_byte(ch)
...
def _pad_right(Str s, int width) -> Str {
    int n = s.len()
    Str r = s
    int i = n
    while i < width {
...
def heading(&!MdDoc d, int level, Str text) {
    int lv = level
    if lv < 1 { lv = 1 }
    if lv > 6 { lv = 6 }
...
def h1(&!MdDoc d, Str text) { d.blocks.push(Heading(1, _inline_text(text))) }
def h2(&!MdDoc d, Str text) { d.blocks.push(Heading(2, _inline_text(text))) }
def h3(&!MdDoc d, Str text) { d.blocks.push(Heading(3, _inline_text(text))) }
def h4(&!MdDoc d, Str text) { d.blocks.push(Heading(4, _inline_text(text))) }
def h5(&!MdDoc d, Str text) { d.blocks.push(Heading(5, _inline_text(text))) }
def h6(&!MdDoc d, Str text) { d.blocks.push(Heading(6, _inline_text(text))) }

def paragraph(&!MdDoc d, Str text) {
    d.blocks.push(Paragraph(_inline_text(text)))
}

def code_block(&!MdDoc d, Str lang, Str code) {
    d.blocks.push(CodeBlock(lang, code))
}

def ul(&!MdDoc d, Vec(Str) items) {
...
def _render_inline(MdInline x) -> Str {
    match x {
...
def _render_inlines(Vec(MdInline) inls) -> Str {
    Str out = ""
    int i = 0
    while i < inls.len {
...
def _render_table(Vec(Str) headers, Vec(Vec(Str)) rows) -> Str {
...
def _render_block(MdBlock b) -> Str {
    match b {
...
def _heading_level(&Str line) -> int {
    int n = line.len()
    int i = 0
...
def _ordered_prefix_len(&Str line) -> int {
    int n = line.len()
    int i = 0
...
def _is_table_sep(Str line) -> bool {
    Str t = line.trim()
    int n = t.len()
    if n == 0 { return false }
    bool has_dash = false
...
def _split_table_row(&Str line) -> Vec(Str) {
...
def _find_char(&Str s, int start, int ch) -> int {
    int n = s.len()
    int i = start
    while i < n {
...
def _find_double(&Str s, int start, int ch) -> int {
    int n = s.len()
    int i = start
    while i + 1 < n {
...
def _find_triple(&Str s, int start, int ch) -> int {
    int n = s.len()
    int i = start
    while i + 2 < n {
...
def _parse_inlines(Str text) -> Vec(MdInline) {
    Vec(MdInline) out = {}
    int n = text.len()
    int i = 0
    Str buf = ""
...
def _parse_blocks(Str input) -> Vec(MdBlock) {
    Vec(MdBlock) blocks = {}
...
def _inline_plain(MdInline x) -> Str {
    match x {
        Text(c)       => { return c.copy() }
...
def _inlines_plain(Vec(MdInline) inls) -> Str {
    Str out = ""
    int i = 0
    while i < inls.len {
...
def _inline_link_of(MdInline x) -> Str {
    match x {
...
def _inline_links(Vec(MdInline) inls) -> Vec(Str) {
    Vec(Str) out = {}
    int i = 0
    while i < inls.len {
...
def _block_links(MdBlock b) -> Vec(Str) {
    match b {
...
def _block_plain(MdBlock b) -> Str {
    match b {
...
def _heading_text(MdBlock b) -> Str {
    match b {
        Heading(_, c)    => { return _inlines_plain(c) }
        Paragraph(_)     => { return "" }
...
def extract_headings(&MdDoc d) -> Vec(Str) {
    Vec(Str) out = {}
    int i = 0
    while i < d.blocks.len {
...
def extract_links(&MdDoc d) -> Vec(Str) {
    Vec(Str) out = {}
    int i = 0
    while i < d.blocks.len {
...
def fmt_heading(int level, Str text) -> Str {
    int lv = level
    if lv < 1 { lv = 1 }
    if lv > 6 { lv = 6 }
...
// std.text.md so the conversion has zero cross-module dependency on std.text.html
// (per docs/plan_std_html.md §5). HTML escaping is inlined here.
// ====================================================================

def _html_escape(Str s) -> Str {
    Str r = ""
    int i = 0
    int n = s.len()
    while i < n {
...
def _html_escape_attr(Str s) -> Str {
    Str r = ""
    int i = 0
    int n = s.len()
    while i < n {
...
def _html_inline(MdInline x) -> Str {
    match x {
...
def _html_inlines(Vec(MdInline) inls) -> Str {
    Str out = ""
    int i = 0
    while i < inls.len {
...
def _html_table(Vec(Str) headers, Vec(Vec(Str)) rows) -> Str {
    Str r = "<table>\n<thead>\n<tr>"
    int c = 0
    while c < headers.len {
        r.push_str("<th>")
