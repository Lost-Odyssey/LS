...
def figure(FigureOpts opts) -> Figure {
    Vec(Axes) al = {}
    return Figure {
        axes_list: al,
        rows: 1, cols: 1,
...
def _nice_number(f64 v, bool round) -> f64 {
    if v == 0.0 { return 0.0 }
    f64 sign = 1.0
    f64 x = v
...
def generate_ticks(f64 lo, f64 hi, int max_ticks) -> Vec(f64) {
    Vec(f64) ticks = {}
...
def map_x(f64 x, f64 xmin, f64 xmax, int left, int width) -> f64 {
    f64 denom = xmax - xmin
    if denom == 0.0 { denom = 1.0 }
    return (left as f64) + (x - xmin) / denom * (width as f64)
}

def map_y(f64 y, f64 ymin, f64 ymax, int top, int height) -> f64 {
...
def line(&!Axes ax, Vec(f64) xs, Vec(f64) ys, LineOpts opts) {
    Str c = opts.color
...
def bar(&!Axes ax, Vec(Str) labels, Vec(f64) values, BarOpts opts) {
    Str c = opts.color
...
def set_title(&!Axes ax, Str t) { ax.title = t }
def set_xlabel(&!Axes ax, Str t) { ax.xlabel = t }
def set_ylabel(&!Axes ax, Str t) { ax.ylabel = t }

def set_xlim(&!Axes ax, f64 lo, f64 hi) {
    ax.auto_scale = false
    ax.xmin = lo
    ax.xmax = hi
}

def set_ylim(&!Axes ax, f64 lo, f64 hi) {
    ax.auto_scale = false
    ax.ymin = lo
    ax.ymax = hi
}

def legend(&!Axes ax) { ax.legend_visible = true }
def grid(&!Axes ax) { ax.grid_visible = true }

// ---- Figure assembly: MOVE an Axes into the Figure ----

def add_axes(&!Figure fig, Axes ax) {
...
def _layout(int sw, int sh) -> Layout {
...
def _render_axes_svg(Axes ax, Layout lo) -> Str {
    Str s = ""
...
def _make_grid(int w, int h) -> Vec(Str) {
    Str row = ""
    int j = 0
...
def _put(&!Vec(Str) g, int col, int row, Str ch) {
...
def _col_text(f64 x, f64 xmin, f64 xmax, int gw) -> int {
    f64 denom = xmax - xmin
    if denom == 0.0 { denom = 1.0 }
...
def _row_text(f64 y, f64 ymin, f64 ymax, int gh) -> int {
    f64 denom = ymax - ymin
    if denom == 0.0 { denom = 1.0 }
...
def _rasterize_line(&!Vec(Str) g, LineStyle ln,
...
def _render_axes_text(Axes ax, int w, int h) -> Str {
    int label_w = 6
    int gw = w - label_w - 1
    int gh = h - 1
    if gw < 1 { gw = 1 }
