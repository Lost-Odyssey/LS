...
import std.chart.plotfmt as plotfmt
...
def event(i64 s, i64 e, Str lane, Str label, Str color) -> TimelineEvent {
    return TimelineEvent { start_ns: s, end_ns: e, lane: lane, label: label, color: color }
}

// ---- helpers ----

def _tl_escape(Str s) -> Str {
    Str r = ""
    int i = 0
    int n = s.len()
    while i < n {
...
def _map_time(i64 t, i64 tmin, i64 tmax, int left, int width) -> f64 {
...
def timeline_svg(Vec(TimelineEvent) events, int w, int h, Str title) -> Str {
    int n = events.len()
...
def timeline_text(Vec(TimelineEvent) events, int w) -> Str {
    int n = events.len()
...
def _to_i64_or(&Str s, i64 dflt) -> i64 {
    match s.to_i64() {
        Ok(v) => { return v }
        Err(e) => { return dflt }
    }
...
def parse_timeline_csv(Str text) -> Vec(TimelineEvent) {
...
def load_timeline_csv(Str path) -> Vec(TimelineEvent) {
...
def cpu_event(i64 s, i64 e, int tid, Str tname, int cpu_id, Str proc) -> CpuSchedEvent {
    return CpuSchedEvent { start_ns: s, end_ns: e, tid: tid, tname: tname, cpu_id: cpu_id, proc: proc }
}

def topology(int logical, int physical) -> CpuTopology {
    int tpc = 1
...
def physical_core(int cpu_id, int total_physical) -> int {
    if total_physical < 1 { return cpu_id }
    return cpu_id % total_physical
}

def is_ht_sibling(int cpu_id, int total_physical) -> bool {
    return cpu_id >= total_physical
}
...
def cpu_hue(int cpu_id, int total_physical) -> f64 {
...
def cpu_color(int cpu_id, int total_physical) -> Str {
...
def _lerp(f64 a, f64 b, f64 t) -> f64 { return a + (b - a) * t }

// viridis-like colormap (5 anchors), frac in [0,1]; mul dims for HT siblings.
def _viridis_hex(f64 frac, f64 mul) -> Str {
    f64 f = frac
    if f < 0.0 { f = 0.0 }
    if f > 1.0 { f = 1.0 }
    f64 r = 0.0
...
def cpu_theme_color(int cpu_id, int total_physical, Str theme) -> Str {
...
def topology2(int total_cpus, bool hyperthreading) -> CpuTopology {
...
def cpu_timeline_text(Vec(CpuSchedEvent) events, int w) -> Str {
    int n = events.len()
    bool first = true
    i64 tmin = 0
    i64 tmax = 1
...
def cpu_timeline_svg(Vec(CpuSchedEvent) events, CpuTopology topo, CpuPlotOpts opts = CpuPlotOpts{}) -> Str {
    int w = opts.w
    int h = opts.h
...
def cpu_timeline_html(Vec(CpuSchedEvent) events, CpuTopology topo, CpuPlotOpts opts = CpuPlotOpts{}) -> Str {
...
def cpu_timeline_html_zoom(Vec(CpuSchedEvent) events, CpuTopology topo, CpuPlotOpts opts = CpuPlotOpts{}) -> Str {
    int w = opts.w
    int h = opts.h
    Str theme = opts.theme
    int n = events.len()
...
def cpu_timeline_aggregated(Vec(CpuSchedEvent) events, CpuTopology topo,
